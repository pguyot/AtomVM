...
%%-----------------------------------------------------------------------------
%% @doc GPIO driver module for generic UNIX (Linux), backed by the sysfs GPIO
%% interface (`/sys/class/gpio').
%%
%% This provides the "Arduino-style" subset of the AtomVM GPIO API
%% (`set_pin_mode/2', `digital_write/2', `digital_read/1') for Linux boards by
%% exporting pins and reading/writing the sysfs attributes through the
%% `atomvm:posix_*' file functions. A pin is the Linux GPIO number.
%%
%% The sysfs base directory defaults to `/sys/class/gpio' and can be overridden
%% with {@link set_sysfs_base/1} (useful for testing). Note: the sysfs GPIO
%% interface is deprecated in recent Linux kernels in favour of the character
%% device (`/dev/gpiochipN'); a libgpiod-based backend can be added later.
%% @end
%%-----------------------------------------------------------------------------
-module(gpio).
...
set_sysfs_base(Dir) ->
    persistent_term:put({?MODULE, sysfs_base}, Dir).

%%-----------------------------------------------------------------------------
%% internal
%%-----------------------------------------------------------------------------
sysfs_base() ->
    persistent_term:get({?MODULE, sysfs_base}, ?DEFAULT_SYSFS_BASE).
...
%% A pin's attribute file, e.g. "/sys/class/gpio/gpio17/value".
attr_path(Pin, Attr) ->
