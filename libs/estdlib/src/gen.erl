...
start(GenMod, LinkP, Module, Args, Options) ->
    do_start(GenMod, LinkP, [self(), Module, Args, Options]).
...
%%
%%          Only `{local, Name}' is supported: AtomVM has no `global' module
%%          and no `via' registry, so OTP's `{global, _}' and `{via, _, _}'
%%          names raise `function_clause'.
%% @end
%%-----------------------------------------------------------------------------
-spec start(
    GenMod :: module(),
...
start(GenMod, LinkP, {local, Name}, Module, Args, Options) when is_atom(Name) ->
    do_start(GenMod, LinkP, [self(), Name, Module, Args, [{name, Name} | Options]]).

%% @private
do_start(GenMod, link, InitArgs) ->
    proc_lib:start_link(GenMod, init_it, InitArgs);
do_start(GenMod, nolink, InitArgs) ->
    proc_lib:start(GenMod, init_it, InitArgs);
do_start(GenMod, monitor, InitArgs) ->
    %% proc_lib:start_monitor/3 returns {Result, MonitorRef}; OTP's gen:start
    %% folds that into {ok, {Pid, Mon}} (or the bare error). The failure-path
    %% 'DOWN' is already drained by proc_lib:start0/7.
    case proc_lib:start_monitor(GenMod, init_it, InitArgs) of
