...
start(Name, Options) when is_atom(Name) andalso is_map(Options) ->
    ok = maps:foreach(
        fun(Key, Val) ->
            case Key of
                name_domain when Val =:= shortnames orelse Val =:= longnames -> ok;
                proto_dist when is_atom(Val) -> ok;
                _ -> error({invalid_option, Key, Val}, [Name, Options])
            end
        end,
        Options
    ),
    net_kernel_sup:start(Options#{name => Name});
...
init(Options) ->
    ...
    {Name, Node} = split_node(NameOption, LongNames),
...
split_node(NameOption, true) ->
    [NamePart, _HostPart] = binary:split(atom_to_binary(NameOption, utf8), <<"@">>, [global]),
    {NamePart, NameOption};
split_node(NameOption, false) ->
    {ok, Hostname} = net:gethostname(),
    {atom_to_list(NameOption), list_to_atom(atom_to_list(NameOption) ++ "@" ++ Hostname)}.
