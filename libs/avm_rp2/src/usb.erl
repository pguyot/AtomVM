...
%%-----------------------------------------------------------------------------
%% @doc USB CDC implementation for RP2040/RP2350 using TinyUSB.
%%
%% This module implements the {@link uart_hal} behaviour over USB CDC ACM,
%% enabling serial distribution over USB connections.
%%
%% The RP2 platform uses TinyUSB for USB device support. When using this
%% driver for distribution, `pico_enable_stdio_usb' should be disabled
%% in the CMake configuration so that the USB CDC interface is available
%% for data transfer instead of console output.
%%
%% Example:
%% ```
%% USB = usb:open([]),
...
%%-----------------------------------------------------------------------------
%% @param Name peripheral name (ignored for RP2 — single CDC interface)
