...
%% {@link set_slave_mode/3},
%% {@link write_blocking/4}, {@link read_blocking/4},
%% {@link write_blocking_until/5}, {@link read_blocking_until/5},
%% {@link write_timeout_us/5}, {@link read_timeout_us/5},
%% {@link write_timeout_per_char_us/5}, {@link read_timeout_per_char_us/5},
%% {@link write_burst_blocking/3}, {@link read_burst_blocking/3},
%% {@link write_raw_blocking/2}, {@link read_raw_blocking/2},
%% {@link get_write_available/1}, {@link get_read_available/1}.
...
%%
%% Note: all blocking I2C operations block the Erlang scheduler for the
%% duration of the I2C transfer. At typical I2C speeds (100-400 kHz), a short
%% transfer completes in hundreds of microseconds. Longer transfers or
%% clock-stretching devices may block for longer.
%% @end
%%-----------------------------------------------------------------------------
-module(i2c).
...
-export([
    ...
    write_blocking/4,
    read_blocking/4,
    write_blocking_until/5,
    read_blocking_until/5,
    write_timeout_us/5,
    read_timeout_us/5,
    write_timeout_per_char_us/5,
    read_timeout_per_char_us/5,
    write_burst_blocking/3,
    read_burst_blocking/3,
    write_raw_blocking/2,
    read_raw_blocking/2,
    get_write_available/1,
    get_read_available/1
]).
...
%% Low-level API (Pico SDK)
%%
%% These NIFs block the Erlang scheduler for the duration of the I2C
%% transfer.  At typical bus speeds (100-400 kHz) a short transfer
%% completes in hundreds of microseconds.
%% ---------------------------------------------------------------------------
...
%%-----------------------------------------------------------------------------
%% @param   Resource I2C resource returned by `init/2'
%% @param   Slave `true' for slave mode, `false' for master mode
%% @param   Addr 7-bit slave address (used in slave mode)
%% @returns `ok'
%% @doc     Set I2C port to slave or master mode.
%% @end
%%-----------------------------------------------------------------------------
-spec set_slave_mode(Resource :: i2c_resource(), Slave :: boolean(), Addr :: non_neg_integer()) ->
    ok.
set_slave_mode(_Resource, _Slave, _Addr) ->
    erlang:nif_error(undefined).
...
%% @param   Data Binary data to write
%% @param   Nostop If true, master retains control of the bus (no Stop issued)
%% @param   UntilUs Absolute deadline in microseconds since boot
%%          (comparable to `erlang:monotonic_time(microsecond)')
%% @returns Number of bytes written, or `{error, Reason}'
%% @doc     Write to I2C device, blocking until an absolute deadline.
%% @end
%%-----------------------------------------------------------------------------
-spec write_blocking_until(
    Resource :: i2c_resource(),
    Addr :: address(),
    Data :: binary(),
    Nostop :: boolean(),
    UntilUs :: non_neg_integer()
) ->
    non_neg_integer() | {error, Reason :: term()}.
write_blocking_until(_Resource, _Addr, _Data, _Nostop, _UntilUs) ->
    erlang:nif_error(undefined).
...
%%          (comparable to `erlang:monotonic_time(microsecond)')
%% @returns `{ok, Data}' or `{error, Reason}'
%% @doc     Read from I2C device, blocking until an absolute deadline.
%% @end
%%-----------------------------------------------------------------------------
-spec read_blocking_until(
    Resource :: i2c_resource(),
    Addr :: address(),
    ...
    {ok, binary()} | {error, Reason :: term()}.
read_blocking_until(_Resource, _Addr, _Count, _Nostop, _UntilUs) ->
    erlang:nif_error(undefined).
...
%% @param   Data Binary data to write
%% @param   Nostop If true, master retains control of the bus (no Stop issued)
%% @param   TimeoutPerCharUs Per-character timeout in microseconds
%% @returns Number of bytes written, or `{error, Reason}'
%% @doc     Write to I2C device, with per-character timeout.
%%
%%          Unlike `write_timeout_us/5' where the timeout covers the entire
%%          transfer, this function applies the timeout to each individual
%%          byte. This is useful for long transfers where clock-stretching
%%          may cause variable per-byte delays.
%% @end
%%-----------------------------------------------------------------------------
-spec write_timeout_per_char_us(
    Resource :: i2c_resource(),
    Addr :: address(),
    Data :: binary(),
    Nostop :: boolean(),
    TimeoutPerCharUs :: non_neg_integer()
) ->
    non_neg_integer() | {error, Reason :: term()}.
write_timeout_per_char_us(_Resource, _Addr, _Data, _Nostop, _TimeoutPerCharUs) ->
    erlang:nif_error(undefined).
...
%% @param   TimeoutPerCharUs Per-character timeout in microseconds
%% @returns `{ok, Data}' or `{error, Reason}'
%% @doc     Read from I2C device, with per-character timeout.
%%
%%          Unlike `read_timeout_us/5' where the timeout covers the entire
%%          transfer, this function applies the timeout to each individual
%%          byte.
%% @end
%%-----------------------------------------------------------------------------
-spec read_timeout_per_char_us(
    Resource :: i2c_resource(),
    Addr :: address(),
    Count :: non_neg_integer(),
    Nostop :: boolean(),
    ...
    {ok, binary()} | {error, Reason :: term()}.
read_timeout_per_char_us(_Resource, _Addr, _Count, _Nostop, _TimeoutPerCharUs) ->
    erlang:nif_error(undefined).
...
%% @param   Data Binary data to write
%% @returns Number of bytes written, or `{error, Reason}'
%% @doc     Write to I2C device in burst mode, blocking.
%%
%%          Burst mode sends data without issuing a Stop condition, and a
%%          subsequent transfer will use a Restart rather than a Start.
%%          This is useful for writing consecutive bytes (e.g. to
%%          sequential registers) without re-addressing the device.
%% @end
%%-----------------------------------------------------------------------------
-spec write_burst_blocking(
    Resource :: i2c_resource(), Addr :: address(), Data :: binary()
) ->
    non_neg_integer() | {error, Reason :: term()}.
write_burst_blocking(_Resource, _Addr, _Data) ->
    erlang:nif_error(undefined).
...
%% @param   Count Number of bytes to read
%% @returns `{ok, Data}' or `{error, Reason}'
%% @doc     Read from I2C device in burst mode, blocking.
%%
%%          Burst mode reads data without issuing a Stop condition, and a
%%          subsequent transfer will use a Restart rather than a Start.
%% @end
%%-----------------------------------------------------------------------------
-spec read_burst_blocking(
    Resource :: i2c_resource(), Addr :: address(), Count :: non_neg_integer()
) ->
    {ok, binary()} | {error, Reason :: term()}.
read_burst_blocking(_Resource, _Addr, _Count) ->
    erlang:nif_error(undefined).
...
%% @param   Data Binary data to write
%% @returns `ok'
%% @doc     Write directly to the I2C TX FIFO, blocking.
%%
%%          This is mainly intended for slave-mode operation. Blocks until
%%          all bytes have been pushed into the TX FIFO.
%% @end
%%-----------------------------------------------------------------------------
-spec write_raw_blocking(Resource :: i2c_resource(), Data :: binary()) -> ok.
write_raw_blocking(_Resource, _Data) ->
    erlang:nif_error(undefined).
...
%% @param   Count Number of bytes to read
%% @returns `{ok, Data}'
%% @doc     Read directly from the I2C RX FIFO, blocking.
%%
%%          This is mainly intended for slave-mode operation. Blocks until
%%          all bytes have been read from the RX FIFO.
%% @end
%%-----------------------------------------------------------------------------
-spec read_raw_blocking(Resource :: i2c_resource(), Count :: non_neg_integer()) ->
    {ok, binary()}.
read_raw_blocking(_Resource, _Count) ->
    erlang:nif_error(undefined).

%%-----------------------------------------------------------------------------
%% @param   Resource I2C resource returned by `init/2'
%% @returns Number of bytes that can be written without blocking
%% @doc     Determine non-blocking write space available in the TX FIFO.
%% @end
%%-----------------------------------------------------------------------------
-spec get_write_available(Resource :: i2c_resource()) -> non_neg_integer().
get_write_available(_Resource) ->
    erlang:nif_error(undefined).

%%-----------------------------------------------------------------------------
%% @param   Resource I2C resource returned by `init/2'
%% @returns Number of bytes available to read without blocking
%% @doc     Determine number of bytes received in the RX FIFO.
%% @end
%%-----------------------------------------------------------------------------
-spec get_read_available(Resource :: i2c_resource()) -> non_neg_integer().
get_read_available(_Resource) ->
    erlang:nif_error(undefined).
