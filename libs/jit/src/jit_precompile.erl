...
compile(Target, Dir, Dwarf, Path) ->
    try
        ...
        <<InfoSize:32, Info:InfoSize/binary>> = jit:beam_chunk_header(
            LabelsCount, Arch, RequestedVariant
        ),
        ...
        Stream2 =
            case Dwarf of
                ...
                false ->
                    Backend:new(
                        RequestedVariant, jit_stream_binary, <<InfoSize:32, Info:InfoSize/binary>>
                    )
            end,
            ...
            case Dwarf of
                true ->
                    ...
                    case jit_dwarf:elf(DwarfStream, NativeCode) of
                        false ->
                            io:format(
                                "Warning: DWARF requested but JIT_DWARF not enabled at compile time~n"
                            ),
                            [{"avmN", <<InfoSize:32, Info:InfoSize/binary, NativeCode/binary>>}];
                        {ok, TextSectionOffset, ELF} ->
                            EmbeddedElfChunk = update_avmn_chunk_with_elf(
                                Info, ELF, TextSectionOffset
                            ),
...
%% Versions (from beam_types.hrl). v3 is OTP < 28, v4 is OTP >= 28: v4 inserts
%% BEAM_TYPE_RECORD at bit 12, pushing the bound/unit flags up by one bit (see
%% the per-version layout in parse_type_entries/3 and src/libAtomVM/module.c).
