...
dp_reg(Cond, Opcode, S, Rd, Rn, Rm) ->
    ...
    RnNum = reg_to_num(Rn),
    RmNum = reg_to_num(Rm),
    Instr = (CondNum bsl 28) bor (Opcode bsl 21) bor (S bsl 20) bor
...
dp_imm(Cond, Opcode, S, Rd, Rn, Imm) ->
    case encode_imm(Imm band 16#FFFFFFFF) of
        {Rotate, Imm8} ->
            ...
            RdNum = reg_to_num(Rd),
            RnNum = reg_to_num(Rn),
            Instr = (CondNum bsl 28) bor (1 bsl 25) bor (Opcode bsl 21) bor (S bsl 20) bor
...
rsb(Cond, Rd, Rn, Rm) when is_atom(Rm) ->
    dp_reg(Cond, 2#0011, 0, Rd, Rn, Rm);
rsb(Cond, Rd, Rn, Imm) when is_integer(Imm) ->
    dp_imm(Cond, 2#0011, 0, Rd, Rn, Imm).

%% AND Rd, Rn, Rm/Imm
-spec and_(cc(), arm_gpr_register(), arm_gpr_register(), arm_gpr_register() | integer()) -> binary().
...
mul(Cond, Rd, Rm, Rs) ->
    ...
    RmNum = reg_to_num(Rm),
    RsNum = reg_to_num(Rs),
    Instr = (CondNum bsl 28) bor (RdNum bsl 16) bor (RsNum bsl 8) bor
        (2#1001 bsl 4) bor RmNum,
    <<Instr:32/little>>.

%% MLS Rd, Rm, Rs, Rn (Rd = Rn - Rm * Rs)
%% Encoding: cond[31:28] 0000 0110 Rd[19:16] Rn[15:12] Rs[11:8] 1001 Rm[3:0]
-spec mls(cc(), arm_gpr_register(), arm_gpr_register(), arm_gpr_register(), arm_gpr_register()) -> binary().
mls(Cond, Rd, Rm, Rs, Rn) ->
    ...
    RsNum = reg_to_num(Rs),
    RnNum = reg_to_num(Rn),
    Instr = (CondNum bsl 28) bor (2#0110 bsl 20) bor (RdNum bsl 16) bor
        (RnNum bsl 12) bor (RsNum bsl 8) bor (2#1001 bsl 4) bor RmNum,
    <<Instr:32/little>>.
...
lsl(Cond, Rd, Rm, Imm) when is_integer(Imm), Imm >= 0, Imm =< 31 ->
    ...
    Instr = (CondNum bsl 28) bor (2#1101 bsl 21) bor (RdNum bsl 12) bor
        (Imm bsl 7) bor RmNum,
    <<Instr:32/little>>;
...
-spec ldr(cc(), arm_gpr_register(), {arm_gpr_register(), integer() | arm_gpr_register()}) -> binary().
ldr(Cond, Rd, {Rn, Offset}) when is_integer(Offset) ->
    ...
    Instr = (CondNum bsl 28) bor (2#01 bsl 26) bor (1 bsl 24) bor (U bsl 23) bor
...
ldr(Cond, Rd, {Rn, Rm}) when is_atom(Rm) ->
    ...
    Instr = (CondNum bsl 28) bor (2#011 bsl 25) bor (1 bsl 24) bor (1 bsl 23) bor
        (1 bsl 20) bor (RnNum bsl 16) bor (RdNum bsl 12) bor RmNum,
    <<Instr:32/little>>.

%% STR Rd, [Rn, #offset] or STR Rd, [Rn, Rm]
-spec str(cc(), arm_gpr_register(), {arm_gpr_register(), integer() | arm_gpr_register()}) -> binary().
...
%% BL{cond} offset - branch with link
%% Encoding: cond[31:28] 1011 imm24[23:0]
-spec bl(cc(), integer()) -> binary().
bl(Cond, Offset) when is_integer(Offset) ->
    CondNum = cond_to_num(Cond),
    AdjustedOffset = Offset - 8,
    0 = AdjustedOffset rem 4,
    Imm24 = AdjustedOffset div 4,
    true = (Imm24 >= -(1 bsl 23)) andalso (Imm24 < (1 bsl 23)),
    Instr = (CondNum bsl 28) bor (2#1011 bsl 24) bor (Imm24 band 16#FFFFFF),
    <<Instr:32/little>>.

%% BX{cond} Rm - branch and exchange
%% Encoding: cond[31:28] 0001 0010 1111 1111 1111 0001 Rm[3:0]
...
%% NOP (MOV r0, r0 for ARMv6 compatibility)
-spec nop() -> binary().
nop() ->
    mov(al, r0, r0).

%% BKPT #imm16
%% Encoding: cond[31:28] 0001 0010 imm12[19:8] 0111 imm4[3:0]
