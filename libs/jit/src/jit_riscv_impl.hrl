...
%% Write-through x-store elision (see jit_backend_pending_impl.hrl). The
%% backend #state must carry live_masks, pending_x and cond_depth fields.
-include("jit_backend_pending_impl.hrl").

%% Receive the per-label live-in x-register masks (jit_liveness pass A) and
%% activate elision. The call-target set is unused (no loop-header residency).
set_live_masks(State, {Masks, _CallTargets}) ->
    State#state{live_masks = Masks}.

%% No loop-header register residency: jit.erl may share op_call_last blocks.
supports_loop_residency() -> false.

%% Nop of a given store width. RISC-V stores are a single word (4 bytes) or,
%% when both registers are compressible and the offset fits, a compressed
%% halfword (2 bytes). The canonical nops: `addi x0, x0, 0' (4) and the
%% compressed `c.nop' (2).
pending_nop_bytes(2) -> ?ASM:c_nop();
pending_nop_bytes(4) -> ?ASM:nop().
...
%% is provided by the includer (jit_riscv32 / jit_riscv64) via
...
%%                  still ours) and the low bits cleared before branching;
%%   bit 1 set too, when TestStay -- JIT_NATIVE_STAY, i.e. resume at the
%%                  instruction after this block.
direct_dispatch(
    ...
    IClearTag = ?ASM:andi(ResultReg, ResultReg, -4),
    ...
    Tail =
        case TestStay of
            true ->
                ITest1 = ?ASM:andi(Temp, ResultReg, 2),
                %% Bit 1 set as well: JIT_NATIVE_STAY, resume at the
                %% instruction after this block.
                IBne1 = branch_over(Temp, byte_size(IClearTag) + byte_size(IJump)),
                <<ITest1/binary, IBne1/binary, IClearTag/binary, IJump/binary>>;
            false ->
...
call_primitive1(State, Primitive, Args, Pure, Available, Regs0, StreamModule, Stream0) ->
    case Available of
        ...
        _ ->
            ...
            Regs1 = jit_regs:invalidate_reg(Regs0, TempReg),
...
call_primitive_last0(
    ...
    #{temp := Temp, available_mask := AvailableRegs1, used_mask := UsedMask} =
    ...
    State1 = State0#state{
        stream = Stream1,
        regs = jit_regs:set_masks(
            jit_regs:invalidate_reg(State0#state.regs, Temp), AvailableRegs1, UsedMask
        )
    },
...
jump_to_label(StateP, Label) ->
    %% Pendings the target may read from memory (its live-in mask) keep their
    %% store; the rest stay pending for a possible flush.
...
branch_to_label_code(
    #state{regs = Regs0, branches = Branches} = State0, Offset, Label, false
) ->
    Available = jit_regs:available_regs(Regs0),
    TempReg =
        case Available of
    ...
    {State1, CodeBlock};
branch_to_label_code(#state{}, _Offset, _Label, LabelLookup) ->
...
%% sequence.
%%
%% A *forward* label's distance is unknown at emit time. On a backtrackable
%% (buffered) stream we emit it optimistically at a size taken from branch_hints
%% (default 4 = a single B-type), record it, and let update_branches resolve it
%% at finalize once every label offset is known; a branch that overflows its
%% reservation is reported in overflows so jit:compile re-emits pinned larger
%% (see the backtrack loop). On a non-backtrackable stream we keep the safe
%% two-branch form. `{'and', _}' keeps if_block.
%% @end
%%-----------------------------------------------------------------------------
...
jump_to_label_cond(#state{labels = Labels} = State0, Cond, Label) ->
    case Labels of
        #{Label := LabelOffset} ->
            ...
            #state{stream_module = StreamModule, regs = Regs} =
            ...
            Code = fused_branch_bytes(
                BranchFunc, Reg, Operand, LabelOffset - BranchOffset, jit_regs:available_regs(Regs)
            ),
            grow_reservation_and_replace(State2, BranchOffset, Code, 4);
        _ ->
...
%% @private
%% Emit a forward fused branch optimistically: reserve branch_hints size (default
%% 4) of 0xFF and record the branch for finalize resolution.
fuse_cond_branch_forward(State0, Cond, Label) ->
    ...
    {State2, {BranchFunc, Reg, Operand}, BranchDelta} = if_block_cond(State1, Cond),
    BranchOffset = Offset0 + BranchDelta,
    %% if_block_cond reserved 4 bytes; grow to the assumed size with 0xFF filler.
    Stream1 =
        case AssumedSize > 4 of
            true ->
                StreamModule:append(State2#state.stream, binary:copy(<<16#FF>>, AssumedSize - 4));
            false ->
                State2#state.stream
        end,
    Fused = {BId, BranchOffset, Label, BranchFunc, Reg, Operand, AssumedSize},
    State2#state{
        stream = Stream1,
...
%% one that does not is reported in overflows and left as its 0xFF placeholder.
resolve_fused_branches(_StreamModule, Stream, [], _Labels, _Avail, Overflows) ->
    {Stream, Overflows};
resolve_fused_branches(
    StreamModule,
    Stream,
    [{BId, BranchOffset, Label, BranchFunc, Reg, Operand, AssumedSize} | Rest],
    Labels,
    Avail,
    Overflows
) ->
    #{Label := LabelOffset} = Labels,
    Code = fused_branch_bytes(BranchFunc, Reg, Operand, LabelOffset - BranchOffset, Avail),
    case byte_size(Code) =< AssumedSize of
        true ->
            ...
            resolve_fused_branches(StreamModule, Stream1, Rest, Labels, Avail, Overflows);
        false ->
            resolve_fused_branches(
                StreamModule, Stream, Rest, Labels, Avail, Overflows#{BId => byte_size(Code)}
            )
    end.

%% @private
%% Pad Code to Size with 4-byte nops on the fall-through side of the branch.
pad_reservation(Code, Size) when byte_size(Code) =:= Size ->
    Code;
pad_reservation(Code, Size) ->
    Nops = binary:copy(?ASM:nop(), (Size - byte_size(Code)) div 4),
    <<Code/binary, Nops/binary>>.

%% @private
%% Overwrite the reserved placeholder at BranchOffset with Code, growing the
%% reservation (0xFF filler, flash bit-clear-safe) when Code exceeds Reserved.
...
%% inverted B-type within ±4 KB, else an inverted-sense skip over a jal (±1 MB)
%% or auipc+jalr jump. Avail is the available-register mask for the far temp.
fused_branch_bytes(BranchFunc, Reg, Operand, Rel, Avail) ->
    if
        ...
        true ->
            ...
            Jump = fused_jump_bytes(Rel - 4, TempReg),
            Skip = apply(?ASM, BranchFunc, [Reg, Operand, 4 + byte_size(Jump)]),
            <<Skip/binary, Jump/binary>>
    end.

%% @private
%% Unconditional jump covering the relative distance JumpRel from the jump
%% instruction: jal within ±1 MB, else auipc+jalr. JumpRel is always outside
%% c.j range here (callers only use this beyond the ±4 KB B-type window), so the
%% size is a deterministic 4 (jal) or 8 (auipc+jalr).
fused_jump_bytes(JumpRel, _TempReg) when JumpRel >= -1048576, JumpRel =< 1048574 ->
    ?ASM:jal(zero, JumpRel);
fused_jump_bytes(JumpRel, TempReg) ->
    Hi20 = (JumpRel + 16#800) bsr 12,
    Lo12Unsigned = JumpRel band 16#FFF,
    Lo12 =
        if
            Lo12Unsigned >= 16#800 -> Lo12Unsigned - 16#1000;
            true -> Lo12Unsigned
        end,
    <<(?ASM:auipc(TempReg, Hi20))/binary, (?ASM:jalr(zero, TempReg, Lo12))/binary>>.
...
if_block_cond(
    ...
    Temp = first_avail(jit_regs:available_regs(State0#state.regs)),
    OffsetBefore = StreamModule:offset(Stream0),
...
if_block_cond(
    ...
    {State2, Temp} = and_(State1, {free, Temp}, Mask),
    Stream2 = State2#state.stream,
    Regs2 = State2#state.regs,
    Avail2 = jit_regs:available_regs(Regs2),
    ...
    case Val of
        0 ->
            ...
            State3 = State2#state{
                stream = Stream3,
                regs = jit_regs:free_reg(Regs2, TempBit)
            },
        ...
        _ ->
            ...
            MaskReg = first_avail(Avail2),
            MaskRegBit = reg_bit(MaskReg),
            State3 = mov_immediate(
                State2#state{
                    regs = jit_regs:alloc_reg(Regs2, MaskRegBit)
                },
                MaskReg,
                Val
            ),
            Stream3 = State3#state.stream,
            Regs3 = State3#state.regs,
            BranchDelta = StreamModule:offset(Stream3) - OffsetBefore,
            BranchInstr = <<16#FFFFFFFF:32/little>>,
            Stream4 = StreamModule:append(Stream3, BranchInstr),
            Regs4 = jit_regs:invalidate_reg(Regs3, MaskReg),
            ReleaseBits = TempBit bor MaskRegBit,
            State4 = State3#state{
                stream = Stream4,
                regs = jit_regs:free_reg(Regs4, ReleaseBits)
            },
...
if_block_cond(
    ...
    AvailRegs = jit_regs:available_regs(Regs0),
    %% RISC-V: AND with mask, then compare with value
    OffsetBefore = StreamModule:offset(Stream0),
    ...
    case Val of
        ...
        _ ->
            ...
            %% Reuse the mask register for the comparison value
            Regs1 = State1#state.regs,
            Avail1 = jit_regs:available_regs(Regs1),
            MaskReg = first_avail(Avail1),
            MaskRegBit = reg_bit(MaskReg),
            State2 = mov_immediate(
                State1#state{
                    regs = jit_regs:alloc_reg(Regs1, MaskRegBit)
                },
                MaskReg,
                Val
            ),
            Stream2 = State2#state.stream,
            Regs2 = State2#state.regs,
            Used2 = jit_regs:used_regs(Regs2),
            BranchDelta = StreamModule:offset(Stream2) - OffsetBefore,
            BranchInstr = <<16#FFFFFFFF:32/little>>,
            Stream3 = StreamModule:append(Stream2, BranchInstr),
            Regs3 = jit_regs:invalidate_reg(Regs2, MaskReg),
            State3 = State2#state{
                stream = Stream3,
                regs = jit_regs:set_masks(
                    Regs3, AvailRegs, Used2 band (bnot MaskRegBit)
                )
            },
...
call_func_ptr0(
    ...
    Regs1 = jit_regs:invalidate_all(State0#state.regs),
    {
        State4#state{
            stream = Stream8,
            regs = jit_regs:set_masks(Regs1, AvailableRegs3Mask, UsedRegs2Mask)
        },
        ResultReg
    }.
...
move_to_vm_register(#state{regs = Regs0} = State0, Src, Dest) ->
    ...
    %% store must persist; a direct native-register store to an x slot
    %% supersedes a same-depth pending store to the same slot. Immediate and
    %% vm-register sources recurse through move_to_vm_register (a nested
    %% native-register store), which does its own bookkeeping.
    StateA =
        case Src of
    ...
    State2 =
        case DirectStore of
            {true, DestX2} ->
                {BaseReg, Off} = ?X_REG(DestX2),
                Width = byte_size(?STORE_WORD(BaseReg, Src, Off)),
...
move_to_vm_register_emit(State0, {y_reg, Y}, Dest) ->
    with_temp(
        ...
        fun(StateT, Temp, AT) ->
            ...
            Stream1 = (StateT#state.stream_module):append(StateT#state.stream, Code),
            % ldr_y_reg clobbers first_avail(AT) as a hidden temp for the Y_REGS pointer
            Regs0a =
                case AT of
                    0 -> StateT#state.regs;
                    _ -> jit_regs:invalidate_reg(StateT#state.regs, first_avail(AT))
                end,
            move_to_vm_register(StateT#state{stream = Stream1, regs = Regs0a}, Temp, Dest)
        end,
...
-spec with_temp(
    state(),
    fun((state(), native_register(), non_neg_integer()) -> state()),
    fun((jit_regs:regs(), native_register()) -> jit_regs:regs())
) -> state().
...
get_array_element(
    ...
    Regs1 = jit_regs:invalidate_reg(Regs0, ElemReg),
    {
        State#state{
            stream = Stream1,
            regs = jit_regs:alloc_reg(Regs1, ElemBit)
        },
        ElemReg
    }.
...
move_to_native_register_emit(
    ...
    Stream1 = StreamModule:append(Stream0, Code),
    {
        State#state{
            stream = Stream1,
            regs = jit_regs:alloc_reg(Regs0, RegABit bor RegBBit)
        },
        {fp, RegA, RegB}
    }.
...
move_to_native_register(
    ...
    AT = jit_regs:available_regs(Regs0),
...
or_(
    ...
    Temp = first_avail(Avail),
    AT = Avail band (bnot reg_bit(Temp)),
    State1 = mov_immediate(State0#state{regs = jit_regs:set_available_regs(Regs0, AT)}, Temp, Val),
    Stream1 = State1#state.stream,
    I = ?ASM:or_(Reg, Temp),
    Stream2 = StreamModule:append(Stream1, I),
    Regs1 = jit_regs:invalidate_reg(jit_regs:invalidate_reg(State1#state.regs, Reg), Temp),
...
addsub_overflow_check(
    ...
    %% When both operands are the same register (e.g. X+X, X-X), it is already
    %% untagged by the srai(Reg) below; untagging it a second time via Val would
    %% shift it right by 8 and corrupt the result.
    UntagVal =
        case Reg =:= Val of
            true -> <<>>;
            false -> ?ASM:srai(Val, Val, 4)
        end,
    Code = <<
...
mul_overflow_check(
    ...
    %% Same-register operands (X*X, squaring) are already untagged by srai(Reg);
    %% re-untagging via Val would corrupt the value (shift right by 8). Also use
    %% Check (not Val) as the sign-extension scratch below: when Reg =:= Val, Val
    %% aliases the low-word result in Reg, so writing it would clobber the product.
    UntagVal =
        case Reg =:= Val of
            true -> <<>>;
            false -> ?ASM:srai(Val, Val, 4)
    ...
    Code = <<
        ...
        (?ASM:srai(Check, Reg, WordBits - 1))/binary,
        (?ASM:xor_(Check, Hi, Check))/binary,
...
mul(
    ...
    AvailAfter = jit_regs:available_regs(State1#state.regs) bor reg_bit(Temp),
    State1#state{
        stream = Stream2,
        regs = jit_regs:set_available_regs(Regs1, AvailAfter)
    };
mul(
...
decrement_reductions_and_maybe_schedule_next(
    ...
    State2#state{stream = StreamN, regs = jit_regs:invalidate_all(State1#state.regs)}.
...
call_only_or_schedule_next(
    ...
    Avail = jit_regs:available_regs(RegsCO),
    Temp = first_avail(Avail),
...
set_cp(#state{regs = RegsSC} = State0) ->
    ...
    State0b = State0#state{
        regs = jit_regs:alloc_reg(RegsSC, TempBit)
    },
