...
move_to_native_register(#state{regs = Regs} = State, Value) ->
    ...
    case Contents =/= unknown andalso jit_regs:find_reg_with_contents(Regs, Contents) of
        {ok, CachedReg} ->
            case State#state.used_regs band reg_bit(CachedReg) of
                0 ->
...
-spec div_reg(state(), x86_64_register(), x86_64_register()) -> {state(), rax}.
div_reg(
...
%% rdx is the native interface pointer and must be saved/restored.
-spec rem_reg(state(), x86_64_register(), x86_64_register()) -> {state(), x86_64_register()}.
rem_reg(
    #state{stream_module = StreamModule, stream = Stream0, regs = Regs0, available_regs = Avail} =
        State,
    DividendReg,
...
%% Convert bitmask to list in reverse allocation order (matching old used_regs list order).
%% Iteration order: rcx, r8, r9, r10, r11, rax (reverse of ?AVAILABLE_REGS).
mask_to_list(0) -> [];
