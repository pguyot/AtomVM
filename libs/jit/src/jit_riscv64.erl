...
%%
%% Note: RISC-V64 instructions are fixed 32-bit with uniform encoding,
%% allowing access to all 32 registers. All registers are 64 bits wide.
%%
...
-type vm_register() ::
    {x_reg, non_neg_integer()} | {y_reg, non_neg_integer()} | {ptr, riscv64_register()}.
...
debugger(#state{stream_module = StreamModule, stream = Stream0} = State) ->
    Stream1 = StreamModule:append(Stream0, jit_riscv64_asm:c_ebreak()),
    State#state{stream = Stream1}.
...
%%-----------------------------------------------------------------------------
-spec used_regs(state()) -> [riscv64_register()].
...
%%-----------------------------------------------------------------------------
-spec available_regs(state()) -> [riscv64_register()].
    ...
    NewInstr =
        case Type of
            {adr, Reg} when Rel rem 4 =:= 0 ->
                ...
                case byte_size(I) of
                    4 -> <<I/binary, (jit_riscv64_asm:nop())/binary>>;
                    6 -> <<I/binary, (jit_riscv64_asm:c_nop())/binary>>;
                    8 -> I
                end;
            ...
            {far_branch, TempReg} ->
                ...
                if
                    Rel >= -1048576 andalso Rel =< 1048574 andalso (Rel rem 2) =:= 0 ->
                        ...
                        DirectBranch = jit_riscv64_asm:jal(zero, Rel),
                        case byte_size(DirectBranch) of
                            2 ->
                                <<DirectBranch/binary, (jit_riscv64_asm:c_nop())/binary,
                                    (jit_riscv64_asm:nop())/binary>>;
                            4 ->
                                <<DirectBranch/binary, (jit_riscv64_asm:nop())/binary>>
                        end;
                    true ->
                        ...
                        I1 = jit_riscv64_asm:auipc(TempReg, Hi20),
                        I2 = jit_riscv64_asm:jalr(zero, TempReg, Lo12),
                        Entry = <<I1/binary, I2/binary>>,
                        case byte_size(Entry) of
                            6 -> <<Entry/binary, (jit_riscv64_asm:c_nop())/binary>>;
                            8 -> Entry
                        end
                end
        end,
...
%%-----------------------------------------------------------------------------
-spec load_primitive_ptr(non_neg_integer(), riscv64_register()) -> binary().
load_primitive_ptr(Primitive, TargetReg) ->
    case Primitive of
        0 ->
            jit_riscv64_asm:ld(TargetReg, ?NATIVE_INTERFACE_REG, 0);
        N when N * 8 < 256 ->
            % Can encode N * 8 directly in li instruction
            I1 = jit_riscv64_asm:li(TargetReg, N * 8),
            I2 = jit_riscv64_asm:add(TargetReg, TargetReg, ?NATIVE_INTERFACE_REG),
            I3 = jit_riscv64_asm:ld(TargetReg, TargetReg, 0),
            <<I1/binary, I2/binary, I3/binary>>;
        N ->
            % For very large primitive numbers, load N and shift left by 3 (multiply by 8)
            I1 = jit_riscv64_asm:li(TargetReg, N),
            I2 = jit_riscv64_asm:slli(TargetReg, TargetReg, 3),
            I3 = jit_riscv64_asm:add(TargetReg, TargetReg, ?NATIVE_INTERFACE_REG),
            I4 = jit_riscv64_asm:ld(TargetReg, TargetReg, 0),
...
%% @return Updated backend state
%%-----------------------------------------------------------------------------
-spec call_primitive(state(), non_neg_integer(), [arg()]) -> {state(), riscv64_register()}.
call_primitive(
    #state{
        stream_module = StreamModule,
        stream = Stream0,
...
return_if_not_equal_to_ctx(
    ...
    I2 =
        case Reg of
            ...
            _ -> jit_riscv64_asm:mv(a0, Reg)
        end,
    I3 = jit_riscv64_asm:ret(),
    % Branch if equal (skip the return)
    % Offset must account for the beq instruction itself (4 bytes) plus I2 and I3
    I1 = jit_riscv64_asm:beq(Reg, ?CTX_REG, 4 + byte_size(I2) + byte_size(I3)),
...
jump_to_offset(
    #state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State, TargetOffset
) ->
    Offset = StreamModule:offset(Stream0),
    CodeBlock = branch_to_offset_code(State, Offset, TargetOffset),
    Stream1 = StreamModule:append(Stream0, CodeBlock),
    State#state{stream = Stream1, regs = jit_regs:invalidate_all(Regs0)}.
...
jump_to_continuation(
    ...
    I2 = jit_riscv64_asm:add(Temp, Temp, OffsetReg),
    % Indirect branch to the calculated absolute address
    I3 = jit_riscv64_asm:jr(Temp),
...
if_block(
    ...
    Stream3 = lists:foldl(
        fun({ReplacementOffset, {BranchFunc, Reg, Operand}}, AccStream) ->
            ...
            NewBranchInstr = apply(jit_riscv64_asm, BranchFunc, [Reg, Operand, BranchOffset]),
...
if_else_block(
    ...
    NewElseJumpInstr = jit_riscv64_asm:j(FinalJumpOffset),
...
%% Delegate (int) forms to regular forms since we only have 64-bit words
...
if_block_cond(
    ...
    I1 = jit_riscv64_asm:slli(Temp, Reg, 63),
    Stream1 = StreamModule:append(Stream0, I1),
    BranchInstr = <<16#FFFFFFFF:32/little>>,
    Stream2 = StreamModule:append(Stream1, BranchInstr),
    State1 = if_block_free_reg(RegOrTuple, State0),
    ...
    TestCode =
        if
            Val >= -2048 andalso Val =< 2047 ->
                ...
                jit_riscv64_asm:andi(Temp, Reg, Val);
            true ->
                %% Need to load immediate into temp register first
                TestCode0 = jit_riscv64_asm:li(Temp, Val),
                TestCode1 = jit_riscv64_asm:and_(Temp, Reg, Temp),
...
if_block_cond(
    ...
    I1 = jit_riscv64_asm:not_(Temp, Reg),
    I2 = jit_riscv64_asm:slli(Temp, Temp, 60),
...
if_block_cond(
    ...
    %% RISC-V: Special case Reg & ?TERM_IMMED_TAG_MASK != ?TERM_INTEGER_TAG
    I1 = jit_riscv64_asm:not_(Reg, Reg),
    I2 = jit_riscv64_asm:slli(Reg, Reg, 60),
    Stream1 = StreamModule:append(Stream0, <<I1/binary, I2/binary>>),
    BranchInstr = <<16#FFFFFFFF:32/little>>,
    Stream2 = StreamModule:append(Stream1, BranchInstr),
    State1 = State0#state{stream = Stream2},
...
if_block_cond(
    ...
    OffsetBefore = StreamModule:offset(Stream0),
    I1 = jit_riscv64_asm:mv(Temp, Reg),
    Stream1 = StreamModule:append(Stream0, I1),
    State1 = State0#state{stream = Stream1},
...
-spec if_block_free_reg(riscv64_register() | {free, riscv64_register()}, state()) -> state().
...
%% @return new state
%%-----------------------------------------------------------------------------
-spec shift_right(#state{}, maybe_free_riscv64_register(), non_neg_integer()) ->
    {#state{}, riscv64_register()}.
shift_right(
    #state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State, {free, Reg}, Shift
) when
    ?IS_GPR(Reg) andalso is_integer(Shift)
->
    I = jit_riscv64_asm:srli(Reg, Reg, Shift),
...
->
    ...
    I = jit_riscv64_asm:srli(ResultReg, Reg, Shift),
    Stream1 = StreamModule:append(Stream0, I),
...
shift_left(
    ...
    is_atom(Reg)
->
    I = jit_riscv64_asm:slli(Reg, Reg, Shift),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
...
shift_right_arith(
    ...
    ?IS_GPR(Reg) andalso is_integer(Shift)
->
    I = jit_riscv64_asm:srai(Reg, Reg, Shift),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    {State#state{stream = Stream1, regs = Regs1}, Reg};
shift_right_arith(
...
->
    ResultReg = first_avail(Avail),
    ResultBit = reg_bit(ResultReg),
    I = jit_riscv64_asm:srai(ResultReg, Reg, Shift),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, ResultReg),
    {
        State#state{
...
%%-----------------------------------------------------------------------------
-spec call_func_ptr(state(), {free, riscv64_register()} | {primitive, non_neg_integer()}, [arg()]) ->
    {state(), riscv64_register()}.
call_func_ptr(
    #state{
        stream_module = StreamModule,
                    ...
                    true ->
                        case SetArgsRegsOnlyAvailableArgs -- ParameterRegs of
                            [] ->
                                ...
                                MovInstr1 = jit_riscv64_asm:mv(NewArgReg, FuncPtrReg1),
                                MovInstr2 = jit_riscv64_asm:mv(FuncPtrReg1, FuncPtrReg0),
                            ...
                            [FuncPtrReg1 | _] ->
                                MovInstr = jit_riscv64_asm:mv(FuncPtrReg1, FuncPtrReg0),
                                SetArgsAvailableArgs1 =
                                    (SetArgsRegsOnlyAvailableArgs -- [FuncPtrReg1]) ++
                                        [FuncPtrReg0],
                                {
    ...
    Call = jit_riscv64_asm:jalr(ra, FuncPtrReg, 0),
...
push_registers(SavedRegs, AlignedStackBytes, StreamModule, Stream0) when length(SavedRegs) > 0 ->
    % RISC-V64: addi sp, sp, -AlignedStackBytes then sd reg, offset(sp) for each reg
    StackAdjust = jit_riscv64_asm:addi(sp, sp, -AlignedStackBytes),
    Stream1 = StreamModule:append(Stream0, StackAdjust),
    {Stream2, _} = lists:foldl(
        fun(Reg, {StreamAcc, Offset}) ->
            Store = jit_riscv64_asm:sd(sp, Reg, Offset),
            {StreamModule:append(StreamAcc, Store), Offset + 8}
        end,
        {Stream1, 0},
        SavedRegs
    ),
...
pop_registers(SavedRegs, AlignedStackBytes, StreamModule, Stream0) when length(SavedRegs) > 0 ->
    % RISC-V64: ld reg, offset(sp) for each reg then addi sp, sp, AlignedStackBytes
    {Stream1, _} = lists:foldl(
        fun(Reg, {StreamAcc, Offset}) ->
            Load = jit_riscv64_asm:ld(Reg, sp, Offset),
            {StreamModule:append(StreamAcc, Load), Offset + 8}
        end,
        {Stream0, 0},
        SavedRegs
    ),
...
parameter_regs0([], _, Acc) ->
    lists:reverse(Acc);
parameter_regs0([{avm_int64_t, _} | T], [Reg | Rest], Acc) ->
...
% Handle 64-bit arguments - LP64 passes 64-bit values in a single register
set_registers_args0(
    State,
...
set_registers_args0(
    ...
    case lists:member(ParamReg, ArgsRegsT) of
        ...
        true ->
            ...
            I = jit_riscv64_asm:mv(Avail, ParamReg),
...
set_registers_args1(
    ...
    case ParamReg of
        a1 ->
            State;
        _ ->
            I = jit_riscv64_asm:mv(ParamReg, a1),
            Stream1 = StreamModule:append(Stream0, I),
            State#state{stream = Stream1}
    end;
...
set_registers_args1(
    ...
    I = jit_riscv64_asm:ld(Reg, BaseReg, Off),
    Stream1 = StreamModule:append(Stream0, I),
    State#state{stream = Stream1};
set_registers_args1(
...
set_registers_args1(
    ...
    ?IS_GPR(ArgReg)
->
    I = jit_riscv64_asm:mv(Reg, ArgReg),
    Stream1 = StreamModule:append(Stream0, I),
    State#state{stream = Stream1};
set_registers_args1(State, Value, Reg, _StackOffset) when ?IS_SIGNED_OR_UNSIGNED_INT64_T(Value) ->
...
move_to_vm_register_emit(State0, Src, {x_reg, extra}) when is_atom(Src) ->
    {BaseReg, Off} = ?X_REG(?MAX_REG),
    I1 = jit_riscv64_asm:sd(BaseReg, Src, Off),
...
move_to_vm_register_emit(State0, Src, {ptr, Reg}) when is_atom(Src) ->
    I1 = jit_riscv64_asm:sd(Reg, Src, 0),
    Stream1 = (State0#state.stream_module):append(State0#state.stream, I1),
    State0#state{stream = Stream1};
...
move_to_vm_register_emit(#state{available_regs = AR0} = State0, {x_reg, extra}, Dest) ->
    ...
    AT = AR0 band (bnot reg_bit(Temp)),
    {BaseReg, Off} = ?X_REG(?MAX_REG),
    I1 = jit_riscv64_asm:ld(Temp, BaseReg, Off),
    Stream1 = (State0#state.stream_module):append(State0#state.stream, I1),
    State1 = move_to_vm_register(State0#state{stream = Stream1, available_regs = AT}, Temp, Dest),
    Regs1 = jit_regs:invalidate_reg(State1#state.regs, Temp),
    State1#state{available_regs = AR0, regs = Regs1};
move_to_vm_register_emit(#state{available_regs = AR0} = State0, {x_reg, X}, Dest) ->
    ...
    AT = AR0 band (bnot reg_bit(Temp)),
    {XReg, X_REGOffset} = ?X_REG(X),
    I1 = jit_riscv64_asm:ld(Temp, XReg, X_REGOffset),
    Stream1 = (State0#state.stream_module):append(State0#state.stream, I1),
    State1 = move_to_vm_register(State0#state{stream = Stream1, available_regs = AT}, Temp, Dest),
    Regs1 = jit_regs:invalidate_reg(State1#state.regs, Temp),
    State1#state{available_regs = AR0, regs = Regs1};
move_to_vm_register_emit(#state{available_regs = AR0} = State0, {ptr, Reg}, Dest) ->
    Temp = first_avail(AR0),
    AT = AR0 band (bnot reg_bit(Temp)),
    I1 = jit_riscv64_asm:ld(Temp, Reg, 0),
    Stream1 = (State0#state.stream_module):append(State0#state.stream, I1),
    State1 = move_to_vm_register(State0#state{stream = Stream1, available_regs = AT}, Temp, Dest),
    Regs1 = jit_regs:invalidate_reg(State1#state.regs, Temp),
    State1#state{available_regs = AR0, regs = Regs1};
...
move_to_vm_register_emit(
    ...
    {BaseReg, Off} = ?FP_REGS,
    I1 = jit_riscv64_asm:ld(Temp1, BaseReg, Off),
    %% On RV64, boxed float layout is: [8-byte header][8-byte double]
    %% The float data starts at offset 8 (= word_size) from the unboxed pointer
    I2 = jit_riscv64_asm:lw(Temp2, Reg, 8),
    case Variant band ?JIT_VARIANT_FLOAT32 of
        0 ->
            % Double precision: write both 32-bit parts
            I3 = jit_riscv64_asm:sw(Temp1, Temp2, F * 8),
            I4 = jit_riscv64_asm:lw(Temp2, Reg, 12),
            I5 = jit_riscv64_asm:sw(Temp1, Temp2, F * 8 + 4),
            Code = <<I1/binary, I2/binary, I3/binary, I4/binary, I5/binary>>;
        _ ->
            % Single precision: write only first 32-bit part
            I3 = jit_riscv64_asm:sw(Temp1, Temp2, F * 4),
            Code = <<I1/binary, I2/binary, I3/binary>>
    end,
    Stream1 = StreamModule:append(Stream0, Code),
...
-spec move_array_element(
    state(),
    riscv64_register(),
    non_neg_integer() | riscv64_register(),
...
move_array_element(
    ...
    Temp2 = first_avail(Avail2),
    AT = Avail2 band (bnot reg_bit(Temp2)),
    I1 = jit_riscv64_asm:ld(Temp2, Reg, Index * 8),
    YCode = str_y_reg(Temp2, Y, Temp1, AT),
...
move_array_element(
    ...
    I1 = jit_riscv64_asm:slli(IndexReg, IndexReg, 3),
    I2 = jit_riscv64_asm:add(IndexReg, Reg, IndexReg),
    I3 = jit_riscv64_asm:ld(IndexReg, IndexReg, 0),
    {BaseReg, Off} = ?X_REG(X),
    I4 = jit_riscv64_asm:sd(BaseReg, IndexReg, Off),
...
move_array_element(
    ...
    I2 = jit_riscv64_asm:add(IndexReg, Reg, IndexReg),
    I3 = jit_riscv64_asm:ld(IndexReg, IndexReg, 0),
    I4 = jit_riscv64_asm:sd(PtrReg, IndexReg, 0),
    Bit = reg_bit(IndexReg),
    AvailableRegs1 = AvailableRegs0 bor Bit,
    UsedRegs1 = UsedRegs0 band (bnot Bit),
    Stream1 = StreamModule:append(Stream0, <<I1/binary, I2/binary, I3/binary, I4/binary>>),
...
-spec get_array_element(
    state(), riscv64_register() | {free, riscv64_register()}, non_neg_integer()
) ->
    {state(), riscv64_register()}.
get_array_element(
    #state{
...
-spec move_to_array_element(
    state(), integer() | vm_register() | riscv64_register(), riscv64_register(), non_neg_integer()
) -> state().
move_to_array_element(
    ...
    I1 = jit_riscv64_asm:sd(Reg, ValueReg, Index * 8),
    Stream1 = StreamModule:append(Stream0, I1),
    State0#state{stream = Stream1};
move_to_array_element(
    #state{stream_module = StreamModule, stream = Stream0, available_regs = Avail} = State0,
    ...
    Temp = first_avail(Avail),
    I1 = jit_riscv64_asm:mv(Temp, IndexReg),
    I2 = jit_riscv64_asm:slli(Temp, Temp, 3),
...
move_to_array_element(
    ...
    Temp = first_avail(Avail),
    I1 = jit_riscv64_asm:addi(Temp, IndexReg, Offset),
    I2 = jit_riscv64_asm:slli(Temp, Temp, 3),
...
move_to_native_register_emit(
    ...
    RegBit = reg_bit(Reg),
    {BaseReg, Off} = ?CP,
    I1 = jit_riscv64_asm:ld(Reg, BaseReg, Off),
    Stream1 = StreamModule:append(Stream0, I1),
...
move_to_native_register_emit(
    ...
    I1 = jit_riscv64_asm:ld(Reg, Reg, 0),
    Stream1 = StreamModule:append(Stream0, I1),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    {State#state{stream = Stream1, regs = Regs1}, Reg};
move_to_native_register_emit(
...
->
    ...
    I1 = jit_riscv64_asm:ld(Reg, BaseReg, Offset),
    Stream1 = StreamModule:append(Stream0, I1),
    Regs1 = jit_regs:set_contents(Regs0, Reg, Contents),
    {
        State#state{
...
move_to_native_register_emit(
    ...
    {BaseReg, Off} = ?FP_REGS,
    I1 = jit_riscv64_asm:ld(RegB, BaseReg, Off),
    I2 = jit_riscv64_asm:lw(RegA, RegB, F * 8),
    I3 = jit_riscv64_asm:lw(RegB, RegB, F * 8 + 4),
    Code = <<I1/binary, I2/binary, I3/binary>>,
    Stream1 = StreamModule:append(Stream0, Code),
    {
        State#state{
...
move_to_native_register(
    ...
    I1 = jit_riscv64_asm:ld(RegDst, Reg, 0),
    Stream1 = StreamModule:append(Stream0, I1),
    State#state{stream = Stream1};
move_to_native_register(
    ...
    {BaseReg, Off} = ?X_REG(?MAX_REG),
    I1 = jit_riscv64_asm:ld(RegDst, BaseReg, Off),
    Stream1 = StreamModule:append(Stream0, I1),
    State#state{stream = Stream1};
move_to_native_register(
...
->
    {XReg, X_REGOffset} = ?X_REG(X),
    I1 = jit_riscv64_asm:ld(RegDst, XReg, X_REGOffset),
    Stream1 = StreamModule:append(Stream0, I1),
    State#state{stream = Stream1};
move_to_native_register(
...
-spec copy_to_native_register(state(), value()) -> {state(), riscv64_register()}.
copy_to_native_register(
    ...
    I1 = jit_riscv64_asm:mv(SaveReg, Reg),
    Stream1 = StreamModule:append(Stream0, I1),
...
copy_to_native_register(
    ...
    SaveReg = first_avail(Avail),
    SaveBit = reg_bit(SaveReg),
    I1 = jit_riscv64_asm:ld(SaveReg, Reg, 0),
    Stream1 = StreamModule:append(Stream0, I1),
...
increment_sp(
    ...
    I1 = jit_riscv64_asm:ld(Reg, BaseReg1, Off1),
    I2 = jit_riscv64_asm:addi(Reg, Reg, Offset * 8),
    {BaseReg2, Off2} = ?Y_REGS,
    I3 = jit_riscv64_asm:sd(BaseReg2, Reg, Off2),
    Code = <<I1/binary, I2/binary, I3/binary>>,
    Stream1 = StreamModule:append(Stream0, Code),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State#state{stream = Stream1, regs = Regs1}.
...
set_continuation_to_label(
    ...
    case lists:keyfind(Label, 1, Labels) of
        {Label, LabelOffset} ->
            ...
            I2 = jit_riscv64_asm:sd(?JITSTATE_REG, Temp, ?JITSTATE_CONTINUATION_OFFSET),
            Code = <<I1/binary, I2/binary>>,
            Stream1 = StreamModule:append(Stream0, Code),
            State#state{stream = Stream1, regs = Regs1};
        false ->
...
get_module_index(
    ...
    Reg = first_avail(Avail),
    RegBit = reg_bit(Reg),
    % Load module pointer from jit_state (which is in a1) - pointer sized
    I1 = jit_riscv64_asm:ld(Reg, ?JITSTATE_REG, ?JITSTATE_MODULE_OFFSET),
    % Load module_index from module struct - 32-bit int
    I2 = jit_riscv64_asm:lw(Reg, Reg, 0),
    Code = <<I1/binary, I2/binary>>,
    Stream1 = StreamModule:append(Stream0, Code),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    {
...
and_(
    ...
    is_atom(SrcReg)
->
    I = jit_riscv64_asm:and_(Reg, Reg, SrcReg),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
...
and_(
    ...
    Val >= -2048 andalso Val =< 2047
->
    I = jit_riscv64_asm:andi(Reg, Reg, Val),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    {State0#state{stream = Stream1, regs = Regs1}, Reg};
and_(
    ...
    I1 = jit_riscv64_asm:not_(Temp, Temp),
    I2 = jit_riscv64_asm:and_(Reg, Reg, Temp),
...
and_(
    ...
    Stream1 = State1#state.stream,
    I = jit_riscv64_asm:and_(Reg, Reg, Temp),
...
and_(
    ...
    % Save a0 to t3
    Save = jit_riscv64_asm:mv(?IP_REG, a0),
    ...
    I1 = jit_riscv64_asm:not_(a0, a0),
    I2 = jit_riscv64_asm:and_(Reg, Reg, a0),
    Stream3 = StreamModule:append(Stream2, <<I1/binary, I2/binary>>),
    % Restore a0 from t3
    Restore = jit_riscv64_asm:mv(a0, ?IP_REG),
...
and_(
    ...
    I = jit_riscv64_asm:and_(Reg, Reg, a0),
...
and_(
    ...
    ResultReg = first_avail(Avail),
    ResultBit = reg_bit(ResultReg),
    I = jit_riscv64_asm:andi(ResultReg, Reg, -4),
    Stream1 = StreamModule:append(State0#state.stream, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, ResultReg),
    {
        State0#state{
...
or_(#state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State0, Reg, SrcReg) when
    is_atom(SrcReg)
->
    I = jit_riscv64_asm:or_(Reg, Reg, SrcReg),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State0#state{stream = Stream1, regs = Regs1};
or_(#state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State0, Reg, Val) when
    Val >= -2048 andalso Val =< 2047
->
    I = jit_riscv64_asm:ori(Reg, Reg, Val),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State0#state{stream = Stream1, regs = Regs1};
or_(
    ...
    State1 = mov_immediate(State0#state{available_regs = AT}, Temp, Val),
    Stream1 = State1#state.stream,
    I = jit_riscv64_asm:or_(Reg, Temp),
    Stream2 = StreamModule:append(Stream1, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
...
xor_(
    ...
    is_atom(SrcReg)
->
    I = jit_riscv64_asm:xor_(Reg, Reg, SrcReg),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State0#state{stream = Stream1, regs = Regs1};
xor_(#state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State0, Reg, Val) when
    Val >= -2048 andalso Val =< 2047
->
    I = jit_riscv64_asm:xori(Reg, Reg, Val),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State0#state{stream = Stream1, regs = Regs1};
xor_(
    ...
    State1 = mov_immediate(State0#state{available_regs = AT}, Temp, Val),
    Stream1 = State1#state.stream,
    I = jit_riscv64_asm:xor_(Reg, Reg, Temp),
    Stream2 = StreamModule:append(Stream1, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State1#state{available_regs = Avail, stream = Stream2, regs = Regs1}.
...
->
    I = jit_riscv64_asm:addi(Reg, Reg, Val),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State0#state{stream = Stream1, regs = Regs1};
add(#state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State0, Reg, Val) when
    is_atom(Val)
->
    I = jit_riscv64_asm:add(Reg, Reg, Val),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State0#state{stream = Stream1, regs = Regs1};
add(#state{stream_module = StreamModule, available_regs = Avail, regs = Regs0} = State0, Reg, Val) ->
    ...
    State1 = mov_immediate(State0#state{available_regs = AT}, Temp, Val),
    Stream1 = State1#state.stream,
    I = jit_riscv64_asm:add(Reg, Reg, Temp),
    Stream2 = StreamModule:append(Stream1, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State1#state{available_regs = Avail, stream = Stream2, regs = Regs1}.
...
sub(#state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State, Reg, Val) when
    Val >= 0 andalso Val =< 255
->
    I1 = jit_riscv64_asm:addi(Reg, Reg, -Val),
    Stream1 = StreamModule:append(Stream0, I1),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State#state{stream = Stream1, regs = Regs1};
sub(#state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State, Reg, Val) when
    is_atom(Val)
->
    I = jit_riscv64_asm:sub(Reg, Reg, Val),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State#state{stream = Stream1, regs = Regs1};
sub(#state{stream_module = StreamModule, available_regs = Avail, regs = Regs0} = State0, Reg, Val) ->
    ...
    State1 = mov_immediate(State0#state{available_regs = AT}, Temp, Val),
    Stream1 = State1#state.stream,
    I = jit_riscv64_asm:sub(Reg, Reg, Temp),
    Stream2 = StreamModule:append(Stream1, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State1#state{available_regs = Avail, stream = Stream2, regs = Regs1}.
...
mul(#state{available_regs = Avail, regs = Regs0} = State, Reg, 3) ->
    Temp = first_avail(Avail),
    I1 = jit_riscv64_asm:slli(Temp, Reg, 1),
    I2 = jit_riscv64_asm:add(Reg, Temp, Reg),
...
mul(#state{available_regs = Avail, regs = Regs0} = State, Reg, 5) ->
    Temp = first_avail(Avail),
    I1 = jit_riscv64_asm:slli(Temp, Reg, 2),
    I2 = jit_riscv64_asm:add(Reg, Temp, Reg),
    Stream1 = (State#state.stream_module):append(State#state.stream, <<I1/binary, I2/binary>>),
    Regs1 = jit_regs:invalidate_reg(jit_regs:invalidate_reg(Regs0, Temp), Reg),
    State#state{stream = Stream1, regs = Regs1};
...
mul(#state{available_regs = Avail, regs = Regs0} = State, Reg, 7) ->
    Temp = first_avail(Avail),
    I1 = jit_riscv64_asm:slli(Temp, Reg, 3),
    I2 = jit_riscv64_asm:sub(Reg, Temp, Reg),
    Stream1 = (State#state.stream_module):append(State#state.stream, <<I1/binary, I2/binary>>),
    Regs1 = jit_regs:invalidate_reg(jit_regs:invalidate_reg(Regs0, Temp), Reg),
    State#state{stream = Stream1, regs = Regs1};
mul(State, Reg, 8) ->
...
mul(#state{available_regs = Avail, regs = Regs0} = State, Reg, 15) ->
    Temp = first_avail(Avail),
    I1 = jit_riscv64_asm:slli(Temp, Reg, 4),
    I2 = jit_riscv64_asm:sub(Reg, Temp, Reg),
    Stream1 = (State#state.stream_module):append(State#state.stream, <<I1/binary, I2/binary>>),
    Regs1 = jit_regs:invalidate_reg(jit_regs:invalidate_reg(Regs0, Temp), Reg),
    State#state{stream = Stream1, regs = Regs1};
...
mul(
    ...
    State1 = mov_immediate(State0#state{available_regs = AT}, Temp, Val),
    Stream1 = State1#state.stream,
    I = jit_riscv64_asm:mul(Reg, Reg, Temp),
    Stream2 = StreamModule:append(Stream1, I),
    Regs1 = jit_regs:invalidate_reg(jit_regs:invalidate_reg(Regs0, Temp), Reg),
    State1#state{
        stream = Stream2,
...
mul(
    ...
    I = jit_riscv64_asm:mul(DestReg, DestReg, SrcReg),
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:invalidate_reg(Regs0, DestReg),
    State#state{stream = Stream1, regs = Regs1}.

%% Register-register multiply: DestReg = DestReg * SrcReg
-spec mul_reg(state(), riscv64_register(), riscv64_register()) -> state().
mul_reg(
    #state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State, DestReg, SrcReg
) ->
    I = jit_riscv64_asm:mul(DestReg, DestReg, SrcReg),
...
%%
%% Analysis of AArch64 pattern and RISC-V64 implementation:
%%
...
%%
%% RISC-V64 implementation (no prolog/epilog needed due to 32 registers):
%%   0x0-0x8:  Decrement reductions, store back
...
%%
%% Key insight: With 32 registers, RISC-V64 doesn't need prolog/epilog like ARM Thumb.
...
decrement_reductions_and_maybe_schedule_next(
    ...
    I2 = jit_riscv64_asm:addi(Temp, Temp, -1),
    % Store back the decremented value
    I3 = jit_riscv64_asm:sw(?JITSTATE_REG, Temp, ?JITSTATE_REDUCTIONCOUNT_OFFSET),
    Stream1 = StreamModule:append(Stream0, <<I1/binary, I2/binary, I3/binary>>),
    ...
    NewI4 = jit_riscv64_asm:bne(Temp, zero, NewOffset - BNEOffset),
    ...
    NewI5 =
        case pc_relative_address(Temp, NewI5Offset) of
            I when byte_size(I) =:= 4 ->
                ...
                <<I/binary, (jit_riscv64_asm:nop())/binary>>;
            I when byte_size(I) =:= 6 ->
                % auipc + c.addi, pad with c.nop (2 bytes)
                <<I/binary, (jit_riscv64_asm:c_nop())/binary>>;
            ...
            {Label, LabelOffset} ->
                ...
                if
                    Rel >= -4096 andalso Rel =< 4094 andalso (Rel rem 2) =:= 0 ->
                        ...
                        I4 = jit_riscv64_asm:bne(Temp, zero, Rel),
                    ...
                    true ->
                        ...
                        I4 = jit_riscv64_asm:beq(Temp, zero, FarSeqSize + 4),
                        Stream2 = StreamModule:append(Stream1, I4),
...
-spec set_cp(state()) -> {state(), non_neg_integer(), riscv64_register()}.
set_cp(#state{available_regs = Avail, used_regs = UsedRegs} = State0) ->
    ...
    I1 = jit_riscv64_asm:slli(Reg, Reg, 24),
    ...
    I4 = jit_riscv64_asm:or_(Reg, TempReg),
    {BaseReg, Off} = ?CP,
    I5 = jit_riscv64_asm:sd(BaseReg, Reg, Off),
    Code = <<I1/binary, I2/binary, I3/binary, I4/binary, I5/binary>>,
    Stream1 = StreamModule:append(Stream0, Code),
    State2 = State1#state{stream = Stream1},
...
-spec rewrite_cp_offset(state(), non_neg_integer(), riscv64_register()) -> state().
rewrite_cp_offset(
    ...
    NewMoveInstr = jit_riscv64_asm:li(TempReg, CPValue),
    ...
    PaddedInstr =
        case byte_size(NewMoveInstr) of
            2 ->
                <<NewMoveInstr/binary, (jit_riscv64_asm:nop())/binary,
                    (jit_riscv64_asm:c_nop())/binary>>;
            4 ->
                <<NewMoveInstr/binary, (jit_riscv64_asm:nop())/binary>>;
            6 ->
                <<NewMoveInstr/binary, (jit_riscv64_asm:c_nop())/binary>>;
            8 ->
                NewMoveInstr
        end,
...
set_bs(
    ...
    Temp = first_avail(Avail),
    {BaseReg1, Off1} = ?BS,
    I1 = jit_riscv64_asm:sd(BaseReg1, TermReg, Off1),
    I2 = jit_riscv64_asm:li(Temp, 0),
...
-spec pc_relative_address(riscv64_register(), integer()) -> binary().
pc_relative_address(Rd, 0) ->
    % Simple case: just get current PC
    jit_riscv64_asm:auipc(Rd, 0);
pc_relative_address(Rd, Offset) ->
    ...
    case {Upper, LowerSigned} of
        ...
        {0, _} ->
            ...
            AuipcInstr = jit_riscv64_asm:auipc(Rd, 0),
            AddiInstr = jit_riscv64_asm:addi(Rd, Rd, LowerSigned),
            <<AuipcInstr/binary, AddiInstr/binary>>;
        {_, 0} ->
            % Only upper bits needed
            jit_riscv64_asm:auipc(Rd, Upper);
        {_, _} ->
            % Both upper and lower bits
            AuipcInstr = jit_riscv64_asm:auipc(Rd, Upper),
            AddiInstr = jit_riscv64_asm:addi(Rd, Rd, LowerSigned),
            <<AuipcInstr/binary, AddiInstr/binary>>
    end.

%% Helper function to generate str instruction with y_reg offset, handling large offsets
str_y_reg(SrcReg, Y, TempReg, _AvailableMask) when Y * 8 =< 248 ->
    % Small offset - use immediate addressing
    {BaseReg, Off} = ?Y_REGS,
    I1 = jit_riscv64_asm:ld(TempReg, BaseReg, Off),
    I2 = jit_riscv64_asm:sd(TempReg, SrcReg, Y * 8),
    <<I1/binary, I2/binary>>;
str_y_reg(SrcReg, Y, TempReg1, AvailableMask) when AvailableMask =/= 0 ->
    ...
    Offset = Y * 8,
    {BaseReg, Off} = ?Y_REGS,
    I1 = jit_riscv64_asm:ld(TempReg1, BaseReg, Off),
    I2 = jit_riscv64_asm:li(TempReg2, Offset),
    I3 = jit_riscv64_asm:add(TempReg2, TempReg2, TempReg1),
    I4 = jit_riscv64_asm:sd(TempReg2, SrcReg, 0),
    <<I1/binary, I2/binary, I3/binary, I4/binary>>;
str_y_reg(SrcReg, Y, TempReg1, 0) ->
    ...
    {BaseReg, Off} = ?Y_REGS,
    I1 = jit_riscv64_asm:ld(TempReg1, BaseReg, Off),
    I2 = jit_riscv64_asm:mv(?IP_REG, TempReg1),
    I3 = jit_riscv64_asm:li(TempReg1, Offset),
    I4 = jit_riscv64_asm:add(TempReg1, TempReg1, ?IP_REG),
    I5 = jit_riscv64_asm:sd(TempReg1, SrcReg, 0),
...
ldr_y_reg(DstReg, Y, AvailableMask) when AvailableMask =/= 0 andalso Y * 8 =< 248 ->
    % Small offset - use immediate addressing
    TempReg = first_avail(AvailableMask),
    {BaseReg, Off} = ?Y_REGS,
    I1 = jit_riscv64_asm:ld(TempReg, BaseReg, Off),
    I2 = jit_riscv64_asm:ld(DstReg, TempReg, Y * 8),
    <<I1/binary, I2/binary>>;
ldr_y_reg(DstReg, Y, AvailableMask) when AvailableMask =/= 0 ->
    ...
    {BaseReg, Off} = ?Y_REGS,
    I1 = jit_riscv64_asm:ld(TempReg, BaseReg, Off),
    I2 = jit_riscv64_asm:li(DstReg, Offset),
    I3 = jit_riscv64_asm:add(DstReg, DstReg, TempReg),
    I4 = jit_riscv64_asm:ld(DstReg, DstReg, 0),
    <<I1/binary, I2/binary, I3/binary, I4/binary>>;
ldr_y_reg(DstReg, Y, 0) when Y * 8 =< 248 ->
    % Small offset, no registers available - use DstReg as temp
    {BaseReg, Off} = ?Y_REGS,
    I1 = jit_riscv64_asm:ld(DstReg, BaseReg, Off),
    I2 = jit_riscv64_asm:ld(DstReg, DstReg, Y * 8),
    <<I1/binary, I2/binary>>;
ldr_y_reg(DstReg, Y, 0) ->
    ...
    {BaseReg, Off} = ?Y_REGS,
    I1 = jit_riscv64_asm:ld(DstReg, BaseReg, Off),
    I2 = jit_riscv64_asm:mv(?IP_REG, DstReg),
    I3 = jit_riscv64_asm:li(DstReg, Offset),
    I4 = jit_riscv64_asm:add(DstReg, DstReg, ?IP_REG),
    I5 = jit_riscv64_asm:ld(DstReg, DstReg, 0),
    <<I1/binary, I2/binary, I3/binary, I4/binary, I5/binary>>.
...
add_label(
    ...
    I1 = jit_riscv64_asm:auipc(a3, Upper20),
    I2 = jit_riscv64_asm:jalr(zero, a3, Lower12Signed),
    % Create 8-byte jump table entry
    JumpTableEntry = <<I1/binary, I2/binary>>,
    PaddedEntry =
        case byte_size(JumpTableEntry) of
            6 -> <<JumpTableEntry/binary, (jit_riscv64_asm:c_nop())/binary>>;
            8 -> JumpTableEntry
        end,
