...
-record(state, {
    ...
    %% Forward fused guard branches (jump_to_label_cond, thumb2 only, on a
    %% backtrackable stream): emitted optimistically at the size from
    ...
    %% them pinned larger. branch_counter gives each a stable id across
    %% re-emits.
...
-define(AVAILABLE_REGS_MASK,
    (?REG_BIT_R7 bor ?REG_BIT_R6 bor ?REG_BIT_R5 bor ?REG_BIT_R4 bor ?REG_BIT_R3 bor ?REG_BIT_R1)
).
-define(SCRATCH_REGS_MASK,
    (?REG_BIT_R7 bor ?REG_BIT_R6 bor ?REG_BIT_R5 bor ?REG_BIT_R4 bor ?REG_BIT_R3 bor
        ?REG_BIT_R2 bor ?REG_BIT_R1 bor ?REG_BIT_R0 bor ?REG_BIT_R12)
).
...
-define(FIRST_AVAIL_REGS, [r7, r6, r5, r4, r3, r1]).
...
%% activate write-through store elision. No loop-header residency on armv6m.
set_live_masks(State, {Masks, _CallTargets}) ->
    State#state{live_masks = Masks}.

supports_loop_residency() -> false.

%% Thumb nop is 2 bytes (`mov r8, r8'); a store may be a 2-byte thumb `str'
%% or a 4-byte thumb2 encoding, so a width-N nop run is N/2 nops.
pending_nop_bytes(N) ->
    Nop = jit_armv6m_asm:nop(),
    binary:copy(Nop, N div 2).
...
call_primitive(StateP, Primitive, Args) ->
    ...
    #state{regs = Regs0} = State = pending_clear_all(StateP),
    Available = jit_regs:available_regs(Regs0),
    call_primitive_impl(Available, State, Primitive, Args).

call_primitive_impl(0, State, Primitive, Args) ->
...
call_primitive_impl(
    ...
    Regs1 = jit_regs:alloc_reg(
        jit_regs:invalidate_reg(Regs0, TempReg),
        TempBit
    ),
...
call_primitive_last0(
    ...
    State5 = State4#state{
        regs = jit_regs:set_masks(
            jit_regs:unreachable(State4#state.regs),
            ?AVAILABLE_REGS_MASK,
            0
        )
    },
...
%% @return Updated backend state
%%-----------------------------------------------------------------------------
jump_to_label(StateP, Label) ->
    %% Pendings the target may read from memory keep their store.
    jump_to_label0(pending_filter_label(StateP, Label), Label).
...
jump_to_offset(StateP, TargetOffset) ->
    %% Entering a shared (tail-cached) block that reads ctx->x: pendings stay.
...
%% @return Updated backend state
%%-----------------------------------------------------------------------------
jump_to_continuation(StateP, {free, OffsetReg}) ->
    jump_to_continuation0(pending_clear_all(StateP), {free, OffsetReg}).

jump_to_continuation0(
    #state{
        stream_module = StreamModule,
        stream = Stream0,
        regs = Regs0,
    ...
    State2 = State1#state{
        stream = Stream2,
        regs = jit_regs:set_masks(
            jit_regs:unreachable(State1#state.regs),
            ?AVAILABLE_REGS_MASK,
            0
        )
    },
...
branch_to_offset_code(
    #state{regs = Regs}, Offset, TargetOffset
) ->
    Available = jit_regs:available_regs(Regs),
    Available =/= 0 orelse error(no_available_registers),
    TempReg = first_avail(Available),
...
branch_to_label_code(
    ...
    {State1, CodeBlock};
branch_to_label_code(#state{regs = Regs} = State0, Offset, Label, false) ->
    branch_to_label_code_impl(jit_regs:available_regs(Regs), State0, Offset, Label).
...
%%
%% Only ARMv7-M (thumb2) does this: it has B<cond>.W with a ±1MB reach and the
%% wide unconditional B.W (±16MB) for the skip form, so a guard fail-jump
%% collapses from a skip-branch plus a far-jump sequence to 2..6 bytes. For a
%% *backward* label the distance is known exactly, so the minimal correct form
%% is emitted directly. A *forward* label's distance is unknown at emit time:
%% on a backtrackable (buffered) stream we emit optimistically at a size taken
%% from branch_hints (default 4 = one B<cond>.W), record it, and let
...
%% non-backtrackable stream we keep the safe two-branch if_block form. The
%% zero-compare conditions that compile to cbz/cbnz (forward-only, 4..130
%% bytes) fuse via inversion when the resolved distance fits, else as a
%% cbz/cbnz skip over a narrow b or B.W. On ARMv6-M (no wide branches) and for
%% `{'and', _}' guards we keep the two-branch if_block form.
%% @end
%%-----------------------------------------------------------------------------
-spec jump_to_label_cond(state(), condition() | {'and', [condition()]}, integer() | reference()) ->
    state().
jump_to_label_cond(#state{thumb2 = false} = State0, Cond, Label) ->
...
jump_to_label_cond(#state{labels = Labels} = State0, Cond, Label) ->
    case Labels of
        #{Label := LabelOffset} ->
            ...
            #state{stream_module = StreamModule} =
            ...
            grow_reservation_and_replace(State2, BranchOffset, Code, 2);
        _ ->
            case backtrack_enabled(State0) of
                true -> fuse_cond_branch_forward(State0, Cond, Label);
                false -> if_block(State0, Cond, fun(BSt0) -> jump_to_label(BSt0, Label) end)
            end
    end.
...
%% (default 4; if_block_cond appended a 2-byte placeholder, grow it with 0xFF
%% filler) and record the branch for finalize resolution in update_branches.
fuse_cond_branch_forward(State0, Cond, Label) ->
    #state{stream_module = StreamModule, branch_counter = BId, branch_hints = Hints} =
        State1 = pending_filter_label(State0, Label),
    ...
    Stream1 = StreamModule:append(
        State2#state.stream, binary:copy(<<16#FF>>, AssumedSize - 2)
    ),
    Fused = {BId, BranchOffset, Label, CC, AssumedSize},
    State2#state{
        stream = Stream1,
        branch_counter = BId + 1,
        fused_branches = [Fused | State2#state.fused_branches]
    }.
...
%% @private
%% Pad Code to Size with 2-byte nops on the fall-through side of the branch.
pad_reservation(Code, Size) when byte_size(Code) =:= Size ->
    Code;
pad_reservation(Code, Size) ->
    Nops = binary:copy(jit_armv6m_asm:nop(), (Size - byte_size(Code)) div 2),
...
%% @private
%% Minimal branch-to-label byte sequence for the exact distance Rel (from the
%% branch instruction address). Atom conditions: a 2-byte Thumb-1 bcc within
%% its ±256B window, a 4-byte B<cond>.W within ±1MB, else a 2-byte skip over a
%% 4-byte unconditional B.W (±16MB, always fits). cbz/cbnz conditions
%% (forward-only, 4..130 from the instruction): the fused inverted 2-byte form
%% when in range, else the skip over a narrow b (±2KB) or B.W. CC is
%% branch-if-false as returned by if_block_cond; single-branch forms invert it.
fused_branch_bytes(CC, Rel) when is_atom(CC), Rel >= -252, Rel =< 258, Rel rem 2 =:= 0 ->
    jit_armv6m_asm:bcc(invert_cc(CC), Rel);
fused_branch_bytes(CC, Rel) when is_atom(CC), Rel - 4 >= -1048576, Rel - 4 =< 1048574 ->
    jit_armv7m_asm:b_w(invert_cc(CC), Rel - 4);
fused_branch_bytes(CC, Rel) when is_atom(CC) ->
    %% Skip branch (2 bytes) over an unconditional B.W at BranchOffset + 2.
    <<(jit_armv6m_asm:bcc(CC, 6))/binary, (jit_armv7m_asm:b_w(Rel - 6))/binary>>;
fused_branch_bytes({CbOp, Reg}, Rel) when Rel >= 4, Rel =< 130, Rel rem 2 =:= 0 ->
    cb_branch(invert_cb(CbOp), Reg, Rel);
fused_branch_bytes({CbOp, Reg}, Rel) when Rel - 2 >= -2044, Rel - 2 =< 2050 ->
    <<(cb_branch(CbOp, Reg, 4))/binary, (jit_armv6m_asm:b(Rel - 2))/binary>>;
fused_branch_bytes({CbOp, Reg}, Rel) ->
    <<(cb_branch(CbOp, Reg, 6))/binary, (jit_armv7m_asm:b_w(Rel - 6))/binary>>.

%% @private
cb_branch(cbz, Reg, Offset) -> jit_armv7m_asm:cbz(Reg, Offset);
cb_branch(cbnz, Reg, Offset) -> jit_armv7m_asm:cbnz(Reg, Offset).
...
if_block_cond(
    ...
    State1 = State0#state{stream = Stream1},
    {State2, Temp} = and_(
        State1#state{regs = jit_regs:set_available_regs(Regs0, AT)}, {free, Temp}, Mask
    ),
    Stream2 = State2#state.stream,
    % Compare with value
    ...
    Regs4 = jit_regs:set_available_regs(
        Regs3, jit_regs:available_regs(State2#state.regs) bor TempBit
    ),
    State3 = State2#state{stream = Stream4, regs = Regs4},
...
if_block_free_reg({free, Reg}, State0) ->
    ...
    Bit = reg_bit(Reg),
    State0#state{regs = jit_regs:free_reg(Regs0, Bit)};
...
->
    ...
    Stream1 = StreamModule:append(Stream0, I),
    Regs1 = jit_regs:alloc_reg(
        jit_regs:invalidate_reg(Regs0, ResultReg),
        Bit
    ),
    {
        State#state{
...
call_func_ptr0(
    ...
    SavedRegsBase = [?CTX_REG, ?NATIVE_INTERFACE_REG | mask_to_list(UsedRegs1Mask)],
    ...
    % Prefer the func ptr register for padding since it's being freed anyway
    {SavedRegs, PaddingReg} =
        case (length(SavedRegsBase) rem 2) =:= 1 of
            true when AvailableRegs1Mask =/= 0 ->
                PaddingReg0 =
                    case FuncPtrTuple of
                        {free, FPR} ->
                            case AvailableRegs1Mask band reg_bit(FPR) =/= 0 of
                                true -> FPR;
                                false when FreeGPMask =/= 0 -> first_avail(FreeGPMask);
                                false -> first_avail(AvailableRegs0Mask)
                            end;
                        _ when FreeGPMask =/= 0 ->
                            first_avail(FreeGPMask);
                        _ ->
                            first_avail(AvailableRegs0Mask)
                    end,
                {SavedRegsBase ++ [PaddingReg0], PaddingReg0};
            _ ->
                {SavedRegsBase, undefined}
        end,
    ...
    AllArgsRegsMask = regs_to_mask(RegArgsRegs ++ StackArgsRegs),
    ...
    SetArgsPushStackMask = (UsedRegs1Mask band (bnot AllArgsRegsMask)) bor AvailableRegs0Mask,
    State1 = State0#state{
        regs = jit_regs:set_masks(
            Regs0,
    ...
    {Stream6, UsedRegs2Mask, ResultReg} =
        case {length(SavedRegs), FuncPtrTuple} of
            {8, {free, ResultFPReg0}} ->
                ...
                StoreResultStackOffset0 = jit_armv6m_asm:reg_to_num(ResultFPReg0) * 4,
                StoreResult0 = jit_armv6m_asm:str(r0, {sp, StoreResultStackOffset0}),
                {
                    StreamModule:append(Stream5, StoreResult0),
                    UsedRegs1Mask bor reg_bit(ResultFPReg0),
                    ResultFPReg0
                };
            {8, _} when PaddingReg =/= undefined ->
                % Use PaddingReg which was saved just for alignment
                StoreResultStackOffset1 = jit_armv6m_asm:reg_to_num(PaddingReg) * 4,
                StoreResult1 = jit_armv6m_asm:str(r0, {sp, StoreResultStackOffset1}),
                {
                    StreamModule:append(Stream5, StoreResult1),
                    UsedRegs1Mask bor reg_bit(PaddingReg),
                    PaddingReg
                };
            {_, {free, ResultFPReg1}} when ResultFPReg1 =/= PaddingReg ->
                % FuncPtrReg is free after the call and not saved, use it for result
                MoveResult0 = jit_armv6m_asm:mov(ResultFPReg1, r0),
                {
                    StreamModule:append(Stream5, MoveResult0),
            ...
            _ ->
                PaddingRegBit =
                    case PaddingReg of
                        undefined -> 0;
                        _ -> reg_bit(PaddingReg)
                    end,
                AvailForResultMask =
                    AvailableRegs1Mask band (bnot (UsedRegs1Mask bor PaddingRegBit)),
                ResultReg0 = first_avail(AvailForResultMask),
                MoveResult0 = jit_armv6m_asm:mov(ResultReg0, r0),
                {
                    StreamModule:append(Stream5, MoveResult0),
    ...
    Regs1 = jit_regs:set_masks(
        jit_regs:invalidate_all(State0#state.regs),
        AvailableRegs3Mask,
        UsedRegs2Mask
    ),
    {
...
set_registers_args(
    ...
    UsedRegsMask = jit_regs:used_regs(Regs0),
...
set_registers_args1(
    ...
    AvailRegs = jit_regs:available_regs(Regs),
...
move_to_vm_register(#state{regs = Regs0} = State0, Src, Dest) ->
    ...
    %% source may be re-read from memory; an x-register destination supersedes
    %% a same-depth pending store. Only the direct native-register store is
    %% noted here (immediate and vm-register sources recurse through a nested
    %% native-register move that notes its own store); the thumb `str' width
    %% (2 or 4 bytes) is measured from the stream.
    StateA =
        case Src of
    ...
    DirectStore =
        case {Src, Dest} of
            {Reg0, {x_reg, DestX0}} when is_atom(Reg0), is_integer(DestX0) -> {true, DestX0};
            _ -> false
        end,
    State =
        case DirectStore of
            {true, DestX1} -> pending_elide_prev(StateA, DestX1);
            false -> StateA
        end,
    ...
    StartOffset = (State#state.stream_module):offset(State#state.stream),
    State1 = move_to_vm_register_emit(State#state{regs = Regs1}, Src, Dest),
    State2 =
        case DirectStore of
            {true, DestX2} -> pending_note_store(State1, DestX2, StartOffset);
            false -> State1
        end,
    case {Src, VmLoc} of
...
move_to_vm_register_emit(
    #state{regs = Regs0} = State0, N, {y_reg, Y}
) when
...
move_to_vm_register_emit(State0, N, Dest) when
    is_integer(N), N >= 0, N =< 255
->
    with_temp(
        State0,
        fun(StateT, Temp, _AT) ->
            ...
            Stream1 = (StateT#state.stream_module):append(StateT#state.stream, I1),
            move_to_vm_register(StateT#state{stream = Stream1}, Temp, Dest)
        end,
        fun(Regs, Temp) -> jit_regs:set_contents(Regs, Temp, {imm, N}) end
    );
...
->
    with_temp(
        State0,
        fun(StateT, Temp, _AT) ->
            State1 = mov_immediate(StateT, Temp, N),
            move_to_vm_register(State1, Temp, Dest)
        end,
        fun(Regs, Temp) -> jit_regs:set_contents(Regs, Temp, {imm, N}) end
    );
% Source is a VM register
move_to_vm_register_emit(State0, {x_reg, extra}, Dest) ->
    with_temp(
        State0,
        fun(StateT, Temp, _AT) ->
        ...
        fun(Regs, Temp) -> jit_regs:set_contents(Regs, Temp, {x_reg, ?MAX_REG}) end
    );
move_to_vm_register_emit(State0, {x_reg, X}, Dest) ->
    with_temp(
        State0,
        fun(StateT, Temp, _AT) ->
        ...
        fun(Regs, Temp) -> jit_regs:set_contents(Regs, Temp, {x_reg, X}) end
    );
move_to_vm_register_emit(State0, {ptr, Reg}, Dest) ->
    with_temp(
        State0,
        fun(StateT, Temp, _AT) ->
        ...
        fun(Regs, Temp) -> jit_regs:invalidate_reg(Regs, Temp) end
    );
move_to_vm_register_emit(State0, {y_reg, Y}, Dest) ->
    with_temp(
        State0,
        fun(StateT, Temp, AT) ->
            State0a = ldr_y_reg(StateT, Temp, Y, AT),
            move_to_vm_register(State0a, Temp, Dest)
        end,
        fun(Regs, Temp) -> jit_regs:set_contents(Regs, Temp, {y_reg, Y}) end
    );
% term_to_float
move_to_vm_register_emit(
    #state{
...
-spec with_temp(
    state(),
    fun((state(), armv6m_register(), non_neg_integer()) -> state()),
    fun((jit_regs:regs(), armv6m_register()) -> jit_regs:regs())
) -> state().
with_temp(#state{regs = Regs0} = State0, EmitFun, ContentsFun) ->
    AR0 = jit_regs:available_regs(Regs0),
    Temp = first_avail(AR0),
    AT = AR0 band (bnot reg_bit(Temp)),
    State1 = EmitFun(State0#state{regs = jit_regs:set_available_regs(Regs0, AT)}, Temp, AT),
    Regs1 = jit_regs:set_available_regs(ContentsFun(State1#state.regs, Temp), AR0),
...
move_array_element(
    ...
    StateE = pending_elide_prev(State, X),
    Avail = jit_regs:available_regs(Regs0),
    Temp1 = first_avail(Avail),
    Temp2 = first_avail(Avail band (bnot reg_bit(Temp1))),
    ...
    State1 = mov_immediate(StateE, Temp1, Remainder),
    Stream1 = State1#state.stream,
    ...
    Regs3 = jit_regs:set_contents(Regs2, Temp2, {x_reg, X}),
    pending_note_store(
        State1#state{stream = Stream2, regs = Regs3},
        X,
        StreamModule:offset(Stream2) - byte_size(I3)
    );
move_array_element(
...
move_array_element(
    #state{
        stream_module = StreamModule, regs = Regs0
    } =
        State,
    Reg,
    Index,
...
move_array_element(
    ...
    Stream1 = StreamModule:append(State#state.stream, <<I1/binary, I2/binary, I3/binary>>),
    Regs1 = jit_regs:invalidate_vm_loc(Regs0, {x_reg, X}),
    Regs2 = jit_regs:free_reg(
        jit_regs:invalidate_reg(Regs1, IndexReg),
        Bit
    ),
    pending_note_store(
        State#state{
        ...
        X,
        StreamModule:offset(Stream1) - byte_size(I3)
    );
move_array_element(
    #state{
        stream_module = StreamModule,
    ...
    Regs1 = jit_regs:free_reg(
        jit_regs:invalidate_reg(Regs0, IndexReg),
        Bit
    ),
    State#state{
        stream = Stream1,
...
move_array_element(
    ...
    TempBit = reg_bit(Temp),
    AT = AvailableRegs0 band (bnot TempBit),
    ...
    Regs3 = jit_regs:free_reg(
        jit_regs:invalidate_reg(Regs2, Temp),
        Bit
    ),
    State#state{
        stream = Stream1,
...
get_array_element(
    ...
    Stream1 = StreamModule:append(Stream0, <<I1/binary>>),
    Regs1 = jit_regs:alloc_reg(
        jit_regs:invalidate_reg(Regs0, ElemReg),
        ElemBit
    ),
    {
        State#state{
...
get_array_element(
    ...
    Regs2 = jit_regs:alloc_reg(
        jit_regs:invalidate_reg(Regs1, Temp),
        ElemBit
    ),
    {
        State1#state{
...
move_to_native_register_emit(
    ...
    State1 = ldr_y_reg(
        State#state{regs = jit_regs:alloc_reg(Regs0, Bit)},
        Reg,
        Y,
        AvailT
    ),
...
move_to_native_register_emit(
    ...
    Stream1 = StreamModule:append(Stream0, Code),
    {
        State#state{
            stream = Stream1,
            regs = jit_regs:alloc_reg(Regs0, BitA bor BitB)
        },
        {fp, RegA, RegB}
    }.
...
move_to_native_register(
    ...
    AT = jit_regs:available_regs(Regs),
...
copy_to_native_register(
    ...
    Regs1 = jit_regs:set_contents(Regs0, SaveReg, SrcContents),
    {
        State#state{
            stream = Stream1,
            regs = jit_regs:alloc_reg(Regs1, SaveBit)
        },
        SaveReg
    };
copy_to_native_register(
...
set_continuation_to_label(
    ...
    %% Re-entry point with unknown x-cache state: commit pending stores.
    #state{
        stream_module = StreamModule,
        stream = Stream0,
...
continuation_entry_point(
    ...
    %% Resumption target: re-entry with unknown x-cache state.
    #state{
        stream_module = StreamModule,
        stream = Stream0
    } = State = pending_clear_all(StateP),
...
and_(
    ...
    Avail = jit_regs:available_regs(Regs0),
    and_imm_impl(Avail, State0, Reg, Val);
and_(
    #state{stream_module = StreamModule, regs = Regs0} =
        State0,
    Reg,
...
and_imm_impl(
    ...
    AT = Avail band (bnot TempBit),
    State1 = mov_immediate(
        State0#state{regs = jit_regs:set_available_regs(Regs0, AT)}, Temp, Val
    ),
    Stream1 = State1#state.stream,
...
or_(
    ...
    or_imm_impl(jit_regs:available_regs(Regs0), State0, Reg, Val).
...
xor_(
    ...
    xor_imm_impl(jit_regs:available_regs(Regs0), State0, Reg, Val).
...
->
    ...
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State0#state{stream = Stream1, regs = Regs1};
add(#state{stream_module = StreamModule, regs = Regs0} = State0, Reg, Val) ->
    Avail = jit_regs:available_regs(Regs0),
    Temp = first_avail(Avail),
    AT = Avail band (bnot reg_bit(Temp)),
    State1 = mov_immediate(
...
->
    ...
    Regs1 = jit_regs:invalidate_reg(Regs0, Reg),
    State#state{stream = Stream1, regs = Regs1};
sub(#state{stream_module = StreamModule, regs = Regs0} = State0, Reg, Val) ->
    Avail = jit_regs:available_regs(Regs0),
    Temp = first_avail(Avail),
    AT = Avail band (bnot reg_bit(Temp)),
    State1 = mov_immediate(
...
mul(
    ...
    Stream2 = StreamModule:append(Stream1, I),
    Regs1 = jit_regs:invalidate_reg(jit_regs:invalidate_reg(State1#state.regs, Temp), Reg),
    State1Avail = jit_regs:available_regs(State1#state.regs),
    State1#state{
        stream = Stream2,
        regs = jit_regs:set_available_regs(Regs1, State1Avail bor TempBit)
    };
mul(
...
decrement_reductions_and_maybe_schedule_next(
    ...
    State2#state{stream = Stream5, regs = jit_regs:invalidate_all(State1#state.regs)}.
