...
% 124
first_pass(
    <<?OP_GC_BIF1, Rest0/binary>>, MMod, MSt, #state{import_resolver = ImportResolver} = State0
) ->
...
% 125
first_pass(
    <<?OP_GC_BIF2, Rest0/binary>>, MMod, MSt, #state{import_resolver = ImportResolver} = State0
) ->
...
% Check if an argument is known to be a binary from type annotations or tracking
is_known_binary(_MMod, _MSt, {typed, _Arg, binary}) ->
    true;
is_known_binary(MMod, MSt, {x_reg, _} = Reg) ->
    MMod:get_type_tracking(MSt, Reg) =:= binary;
is_known_binary(MMod, MSt, {y_reg, _} = Reg) ->
    MMod:get_type_tracking(MSt, Reg) =:= binary;
...
op_gc_bif2(
    ...
    Range2 = {Arg2Value, Arg2Value},
    op_gc_bif2_rem(MMod, MSt0, FailLabel, Live, Bif, Arg1, Arg2, Dest, Range1, Range2);
% element - typed integer index with literal index >= 1
op_gc_bif2(
    MMod,
    MSt0,
    FailLabel,
    ...
    element,
    Arg1,
    {typed, Arg2, _TupleType},
    Dest
) when is_integer(Arg1), Arg1 band ?TERM_IMMED_TAG_MASK =:= ?TERM_INTEGER_TAG, Arg1 bsr 4 >= 1 ->
    op_gc_bif2_element_literal(MMod, MSt0, FailLabel, Arg1, Arg2, Dest);
...
->
    case can_inline_mul(Range1, Range2, MMod) of
        true ->
            ...
            case Arg2Value of
                0 ->
                    % Result is always tagged zero
                    {MSt1, Reg} = MMod:move_to_native_register(MSt0, ?TERM_INTEGER_TAG),
                ...
                _ ->
                    % Negative constant - fall back to default
                    op_gc_bif2_default(MMod, MSt0, FailLabel, Live, Bif, Arg1, Arg2, Dest)
            end;
        false ->
            op_gc_bif2_default(MMod, MSt0, FailLabel, Live, Bif, Arg1, Arg2, Dest)
...
op_gc_bif2_mul(MMod, MSt0, FailLabel, Live, Bif, Arg1, Arg2, Dest, Range1, Range2) ->
    case can_inline_mul(Range1, Range2, MMod) of
        true ->
            ...
            MSt6 = MMod:mul_reg(MSt5, Reg1, Reg2),
            % Add tag back
...
% Only safe for non-negative inputs (logical shift is used)
can_inline_bsr(Range1, ShiftAmount, MMod) ->
    ...
    case Range1 of
        ...
        ->
            % Non-negative input: result always fits if input fits
            Min1 >= MinSafe andalso Max1 =< MaxSafe;
        _ ->
            false
    end.
        ...
        true ->
            case ShiftAmount of
                ...
                _ ->
                    ...
                    % The tag bits (0xF) get shifted away since S+4 >= 4
                    {MSt1, Reg} = MMod:move_to_native_register(MSt0, Arg1),
...
can_inline_div(Range1, Range2, MMod) ->
    case erlang:function_exported(MMod, div_reg, 3) of
        false ->
            false;
        true ->
            {MinSafe, MaxSafe} =
...
->
    case can_inline_div(Range1, Range2, MMod) of
        true ->
            ...
            {MSt5, QuotientReg} = MMod:div_reg(MSt4, Reg1, Reg2),
...
op_gc_bif2_div(MMod, MSt0, FailLabel, Live, Bif, Arg1, Arg2, Dest, Range1, Range2) ->
    case can_inline_div(Range1, Range2, MMod) of
        true ->
            ...
            {MSt7, QuotientReg} = MMod:div_reg(MSt6, Reg1, Reg2),
...
->
    case can_inline_div(Range1, Range2, MMod) of
        true ->
            ...
            {MSt3, Reg1} = MMod:shift_right_arith(MSt2, {free, Reg1}, 4),
            {MSt4, Reg2} = MMod:move_to_native_register(MSt3, Arg2Value),
            {MSt5, RemReg} = MMod:rem_reg(MSt4, Reg1, Reg2),
...
op_gc_bif2_rem(MMod, MSt0, FailLabel, Live, Bif, Arg1, Arg2, Dest, Range1, Range2) ->
    case can_inline_div(Range1, Range2, MMod) of
        true ->
            ...
            {MSt5, Reg2} = MMod:and_(MSt4, {free, Reg2}, bnot (?TERM_IMMED_TAG_MASK)),
            {MSt6, Reg2} = MMod:shift_right_arith(MSt5, {free, Reg2}, 4),
            {MSt7, RemReg} = MMod:rem_reg(MSt6, Reg1, Reg2),
...
op_gc_bif2_element_literal(MMod, MSt0, FailLabel, Arg1, Arg2, Dest) ->
    Index = Arg1 bsr 4,
    {MSt1, TupleReg} = MMod:move_to_native_register(MSt0, Arg2),
    {MSt2, TupleReg} = MMod:and_(MSt1, {free, TupleReg}, ?TERM_PRIMARY_CLEAR_MASK),
    % Load arity from tuple header word (element 0)
    {MSt3, ArityReg} = MMod:get_array_element(MSt2, TupleReg, 0),
    {MSt4, ArityReg} = MMod:shift_right(MSt3, {free, ArityReg}, 6),
    % Bounds check: fail if arity < index
    MSt5 = cond_raise_badarg_or_jump_to_fail_label(
        {{free, ArityReg}, '<', Index}, FailLabel, MMod, MSt4
    ),
    % Load element at 1-based index (element 0 is the header)
    MSt6 = MMod:move_array_element(MSt5, TupleReg, Index, Dest),
    MMod:free_native_registers(MSt6, [TupleReg, ArityReg, Dest]).
...
collect_get_tuple_elements(
    ...
    case Source =:= SrcArg of
        true when Dest =:= SrcArg ->
            ...
            ?TRACE("FUSE: + get_tuple_element ~p, ~p, ~p (last, overwrites source)\n", [Source, Element, Dest]),
...
emit_fused_tuple_ops(IsTupleLabel, TestArityLabel, Arg1, Arity, GetElements, MMod, MSt0) ->
    ...
    MSt6 = ?DWARF_OPCODE(MMod, MSt5, <<"test_arity/3">>),
    {MSt7, ArityReg} = MMod:shift_right(MSt6, {free, HeaderReg}, 6),
    MSt8 = cond_jump_to_label({{free, ArityReg}, '!=', Arity}, TestArityLabel, MMod, MSt7),
    MSt9 = lists:foldl(
        fun({Element, Dest}, AccMSt0) ->
            AccMSt1 = ?DWARF_OPCODE(MMod, AccMSt0, <<"get_tuple_element/3">>),
            AccMSt2 = MMod:move_array_element(AccMSt1, Reg, Element + 1, Dest),
            MMod:free_native_registers(AccMSt2, [Dest])
        end,
        MSt8,
        GetElements
    ),
    MSt10 = MMod:free_native_registers(MSt9, [Reg]),
    MSt11 = set_type_after_guard(MMod, MSt10, Arg1, tuple),
    ?ASSERT_ALL_NATIVE_FREE(MSt11),
    MSt11.
