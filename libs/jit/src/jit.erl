        ...
        fun(BSt0) ->
            MMod:if_else_block(
                ...
                fun(BSt1) ->
                    ...
                    %% code may read the result register directly). Strip a copy
                    %% of operand 2's tag; R1 keeps its tag so the result is
                    %% correctly tagged. On overflow R1 is clobbered, but the
                    %% fallback re-reads the original VM operands.
                    {BSt2, Tmp} = MMod:copy_to_native_register(BSt1, R2),
                    {BSt3, TmpS} = MMod:and_(BSt2, {free, Tmp}, bnot (?TERM_IMMED_TAG_MASK)),
                    BSt4 =
                        case Op of
