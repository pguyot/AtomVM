    ...
    {MSt6, NumBits} =
        if
            ...
            true ->
                ...
                %% the match proceed before the start of the bitstring.
...
emit_pass(<<?OP_BS_GET_BINARY2, Rest0/binary>>, MMod, MSt0, State0) ->
    ...
    {MSt14, SizeBits} =
        if
            Size =:= ?ALL_ATOM ->
                ...
                % the segment unit
                MSt11 = bs_jump_unless_multiple_of_unit(RemainingReg, Unit, Fail, MMod, MSt10),
                {MSt11, RemainingReg};
            is_integer(Size) ->
            ...
            true ->
                ...
                MSt12 = MMod:if_else_block(
                    ...
                    {SizeValReg, '==', ?ALL_ATOM},
                    fun(BSt0) ->
                        BSt1 = bs_jump_unless_multiple_of_unit(
                    ...
                    fun(BSt0) ->
                        ...
                        %% A negative size fails the match (nomatch), as on BEAM.
    ...
    {MSt28, Dest, Rest7} = decode_dest(Rest6, MMod, MSt27),
    ?TRACE("OP_BS_GET_BINARY2 ~p,~p,~p,~p,~p,~p,~p\n", [
        Fail, Src, Live, Size, Unit, FlagsValue, Dest
    ]),
    MSt29 = MMod:move_to_vm_register(MSt28, ResultTerm, Dest),
    MSt30 = MMod:free_native_registers(MSt29, [ResultTerm, Dest]),
    ?ASSERT_ALL_NATIVE_FREE(MSt30),
    emit_pass(Rest7, MMod, MSt30, State0);
% 120
...
emit_pass(<<?OP_BS_TEST_UNIT, Rest0/binary>>, MMod, MSt0, State0) ->
    ...
    MSt9 = bs_jump_unless_multiple_of_unit(BSBinarySize, Unit, Fail, MMod, MSt8),
...
emit_pass(
    ...
    {MSt14, BinaryTotalSizeInBytes, AllocSize, WrapInfo} =
        if
            ...
            true ->
                ...
                {MSt9, CeilBytes} = MMod:shift_right(MSt8, {free, BinaryTotalSize}, 3),
                {MSt10, CeilCopy} = MMod:copy_to_native_register(MSt9, CeilBytes),
                {MSt11, AllocSizeReg} = term_binary_heap_size({free, CeilCopy}, MMod, MSt10),
                MSt12 = MMod:add(MSt11, AllocSizeReg, ?TERM_BOXED_SUB_BINARY_SIZE),
                MSt13 =
                    case Alloc of
                        0 -> MSt12;
                        _ -> MMod:add(MSt12, AllocSizeReg, Alloc)
                    end,
                {MSt13, CeilBytes, AllocSizeReg, wrap_dyn}
        end,
    {MSt15, MemoryEnsureFreeReg} = MMod:call_primitive(
        MSt14, ?PRIM_MEMORY_ENSURE_FREE_WITH_ROOTS, [
            ctx, jit_state, {free, AllocSize}, Live, ?MEMORY_CAN_SHRINK
        ]
    ),
    MSt16 = handle_error_if({'(bool)', {free, MemoryEnsureFreeReg}, '==', false}, MMod, MSt15),
    {MSt19, InitialCreatedBin} =
        case ReuseSourceBinary of
            false ->
                % No reuse - create the binary now
                {MSt17, CreatedBinResult} = MMod:call_primitive(
                    MSt16, ?PRIM_TERM_CREATE_EMPTY_BINARY, [
                        ctx, {free, BinaryTotalSizeInBytes}
                    ]
                ),
                MSt18 = MMod:if_block(MSt17, {CreatedBinResult, '==', ?TERM_INVALID_TERM}, fun(
                    BSt0
                ) ->
                    MMod:call_primitive_last(BSt0, ?PRIM_RAISE_ERROR, [
                        ctx, jit_state, offset, ?OUT_OF_MEMORY_ATOM
    ...
    {Rest7, MSt20, FinalOffset, CreatedBin} = lists:foldl(
        ...
        {Rest6, MSt19, 0, InitialCreatedBin},
        lists:seq(1, NBSegments)
    ),
    ?TRACE("]\n", []),
    ...
    {MSt21, CreatedBin1} =
        case WrapInfo of
            no_wrap ->
                {MSt20, CreatedBin};
            {wrap_lit, TotalBits} ->
                MMod:call_primitive(MSt20, ?PRIM_BS_CREATE_BIN_WRAP, [
    ...
    MSt22 = MMod:free_native_registers(MSt21, [FinalOffset]),
    MSt23 = MMod:move_to_vm_register(MSt22, CreatedBin1, Dest),
    MSt24 = MMod:free_native_registers(MSt23, [CreatedBin1, Dest]),
    ?ASSERT_ALL_NATIVE_FREE(MSt24),
    emit_pass(Rest7, MMod, MSt24, State1);
% 178
...
emit_pass_bs_create_bin_compute_size(
    ...
    {MSt3a, Reg} = term_bit_size({free, Reg0}, MMod, MSt2),
    %% The whole source is taken, so its bit size must be a multiple of the
    %% segment unit: a /binary segment (unit 8) rejects a partial bitstring,
    %% a /bitstring one (unit 1) takes it.
    MSt3 = bs_badarg_unless_multiple_of_unit(Reg, SegmentUnit, Fail, MMod, MSt3a),
    case AccSizeReg0 of
        undefined ->
            {MSt3, AccLiteralSize0, Reg, State0};
        _ ->
            MSt4 = MMod:add(MSt3, AccSizeReg0, Reg),
...
->
    ...
    %% Verify the source provides at least RequiredBits bits
...
emit_pass_bs_create_bin_compute_size(
    ...
    {MSt3, Reg1a} = MMod:copy_to_native_register(MSt2, Src),
    {MSt4, Reg1} = term_bit_size({free, Reg1a}, MMod, MSt3),
    ...
    case AccSizeReg0 of
        undefined ->
            {MSt5, AccLiteralSize0, Reg1, State0};
        _ ->
            MSt6 = MMod:add(MSt5, AccSizeReg0, Reg1),
            MSt7 = MMod:free_native_registers(MSt6, [Reg1]),
            ...
            MSt7 =
                if
                    Unit > 1 ->
                        ...
                        MSt5 = bs_jump_unless_multiple_of_unit(Reg, Unit, Fail, MMod, MSt4),
...
emit_pass_bs_match_binary(
    ...
    {MSt12, Dest, Rest5} = decode_dest(Rest4, MMod, MSt11),
    ?TRACE("~p},", [Dest]),
    MSt13 = MMod:move_to_vm_register(MSt12, ResultTerm, Dest),
    MSt14 = MMod:free_native_registers(MSt13, [ResultTerm]),
    MSt15 = MMod:add(MSt14, BSOffsetReg, MatchedBits),
    {J0 - 5, Rest5, NewMatchState, BSOffsetReg, MSt15}.
...
%% Jump to Fail unless BitsReg is a multiple of Unit. Backends have no division,
%% so a power-of-two unit tests the remainder with a mask and any other unit goes
%% through a primitive; the interpreter tests `remaining_bits rem unit` for every
%% unit, and the two must agree.
bs_jump_unless_multiple_of_unit(_BitsReg, 1, _Fail, _MMod, MSt0) ->
    MSt0;
bs_jump_unless_multiple_of_unit(BitsReg, Unit, Fail, MMod, MSt0) when Unit band (Unit - 1) =:= 0 ->
    cond_jump_to_label({BitsReg, '&', Unit - 1, '!=', 0}, Fail, MMod, MSt0);
bs_jump_unless_multiple_of_unit(BitsReg, Unit, Fail, MMod, MSt0) ->
    {MSt1, ResultReg} = MMod:call_primitive(MSt0, ?PRIM_BITSTRING_IS_MULTIPLE_OF, [
        BitsReg, Unit
    ]),
    cond_jump_to_label({'(bool)', {free, ResultReg}, '==', false}, Fail, MMod, MSt1).

%% Same divisibility test as bs_jump_unless_multiple_of_unit, but raising badarg
%% when there is no fail label (construction, where BEAM raises rather than
%% failing a match).
bs_badarg_unless_multiple_of_unit(_BitsReg, Unit, _Fail, _MMod, MSt0) when Unit =:= 1 ->
    MSt0;
bs_badarg_unless_multiple_of_unit(BitsReg, Unit, Fail, MMod, MSt0) when
    Unit band (Unit - 1) =:= 0
->
    cond_raise_badarg_or_jump_to_fail_label(
        {BitsReg, '&', Unit - 1, '!=', 0}, Fail, MMod, MSt0
    );
bs_badarg_unless_multiple_of_unit(BitsReg, Unit, Fail, MMod, MSt0) ->
    {MSt1, ResultReg} = MMod:call_primitive(MSt0, ?PRIM_BITSTRING_IS_MULTIPLE_OF, [
        BitsReg, Unit
    ]),
    cond_raise_badarg_or_jump_to_fail_label(
