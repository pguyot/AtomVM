...
%% ARM32 (ARMv6) AAPCS32 ABI: r0-r3 are used for argument passing and return value.
...
%%
%% Registers used by the JIT backend (ARM32 ARM mode):
...
%% ARM32 (ARMv6) register allocation:
...
load_primitive_ptr(Primitive, TargetReg) ->
    case Primitive of
        ...
        N ->
            ...
            I3 = jit_arm32_asm:ldr(al, TargetReg, {?NATIVE_INTERFACE_REG, TargetReg}),
...
-spec if_block_cond(state(), condition()) ->
    {
        state(),
        jit_arm32_asm:cc() | {tbz | tbnz, atom(), 0..63} | {cbz, atom()},
        non_neg_integer()
    }.
    ...
    {Stream6, UsedRegs2} =
        case length(SavedRegs) of
            8 when element(1, FuncPtrTuple) =:= free ->
                ...
                StoreResultStackOffset = jit_arm32_asm:reg_to_num(ResultReg) * 4,
...
pop_registers(SavedRegs, StreamModule, Stream0) when length(SavedRegs) > 0 ->
    Stream1 = StreamModule:append(Stream0, jit_arm32_asm:pop(SavedRegs)),
    Stream1;
...
-spec mul_reg(state(), arm32_register(), arm32_register()) -> state().
mul_reg(
...
%%
%% Analysis of AArch64 pattern and ARM32 ARM mode mapping:
%%
...
%%   continuation: push {r1,r4-r11,lr}        ; PROLOG (only executed when scheduled)
...
rewrite_cp_offset(
    ...
    % ARM32: all instructions are 4-byte aligned, no padding needed
...
return_labels_and_lines(
    ...
    % ARM32: all instructions are 4-byte aligned
...
str_y_reg(SrcReg, Y, TempReg, _AvailMask) when Y * 4 =< 4095 ->
...
str_y_reg(SrcReg, Y, TempReg1, _AvailMask) ->
    % Large offset (Y * 4 > 4095) - split into base + 4092 + remainder
    Offset = Y * 4,
    BaseOffset = 4092,
...
ldr_y_reg(DstReg, Y, 0) ->
    ...
    % Use DstReg as temp: load Y_REGS base, add 4092, ldr with remainder
    Offset = Y * 4,
    BaseOffset = 4092,
    Remainder = Offset - BaseOffset,
    I1 = jit_arm32_asm:ldr(al, DstReg, ?Y_REGS),
...
add_label(#state{stream_module = StreamModule, stream = Stream0, regs = Regs0} = State0, Label) ->
    ...
    add_label(State0#state{regs = Regs2}, Label, Offset0).
