...
- Added support for the OTP 30 `get_record_elements_id/4` and `update_record_id/6` opcodes
...
- Added `erlang:term_to_binary/2`, `erlang:is_builtin/3` and `erlang:bitstring_to_list/1`
- Added an OTP-style `application` controller and master, replacing the `application` stub with
...
- Reduced the size of JIT-generated code on every backend by emitting the fallback of the
