...
isolated(Fun) ->
    Parent = self(),
    Pid = spawn(fun() -> Fun(), Parent ! {done, self()} end),
    receive {done, Pid} -> ok after 60000 -> error(probe_timeout) end.
...
fill(0) -> ok;
fill(N) -> self() ! old_message, fill(N - 1).

request_loop(0, Sum) -> Sum;
request_loop(N, Sum) ->
    Ref = make_ref(),
    self() ! {Ref, 1},
    receive {Ref, Value} -> request_loop(N - 1, Sum + Value)
    after 5000 -> error(request_timeout)
    end.

drain(N) ->
    receive old_message -> drain(N + 1) after 0 -> N end.

iterator_probe(N, Take) ->
    Map = make_map(N, #{}),
    erlang:garbage_collect(),
    Repeats = case N =:= Take of true -> 10; false -> 1000 end,
    T0 = erlang:monotonic_time(microsecond),
