...
static uint16_t get_mem_add_size(term mem_add_size_term)
{
    avm_int_t val = term_to_int(mem_add_size_term);
    if (val == 16) {
...
static term nif_i2c_mem_read(Context *ctx, int argc, term argv[])
{
    ...
    uint16_t mem_addr_size = get_mem_add_size(argv[3]);
    avm_int_t count = term_to_int(argv[4]);
    uint32_t timeout_ms = get_timeout_ms(argv[5]);
...
static term nif_i2c_is_device_ready(Context *ctx, int argc, term argv[])
{
    ...
    uint16_t addr = (uint16_t) (term_to_int(argv[1]) << 1);
    uint32_t trials = (uint32_t) term_to_int(argv[2]);
    uint32_t timeout_ms = get_timeout_ms(argv[3]);

    HAL_StatusTypeDef status = HAL_I2C_IsDeviceReady(&rsrc_obj->handle, addr, trials, timeout_ms);
    if (UNLIKELY(status != HAL_OK)) {
        if (UNLIKELY(memory_ensure_free(ctx, TUPLE_SIZE(2)) != MEMORY_GC_OK)) {
            RAISE_ERROR(OUT_OF_MEMORY_ATOM);
        }
