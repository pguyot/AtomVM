...
void sys_monotonic_time(struct timespec *t)
{
    ...
    // mach_absolute_time (a commpage read). BEAM's monotonic clock also uses
    // mach_absolute_time on macOS. The scheduler reads this clock on every
    // timer arm/expiry check, so it is on the message-passing hot path.
