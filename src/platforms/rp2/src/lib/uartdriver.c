...
static term nif_uart_init(Context *ctx, int argc, term argv[])
{
    ...
    int baudrate_val = term_to_int(argv[1]);
    if (UNLIKELY(baudrate_val <= 0)) {
        RAISE_ERROR(BADARG_ATOM);
    }
    uint baudrate = (uint) baudrate_val;
    uart_inst_t *inst = uart_get_instance((uint) peripheral);

    if (UNLIKELY(uart_is_enabled(inst))) {
        RAISE_ERROR(BADARG_ATOM);
    }
    ...
    term_put_tuple_element(inner, 0, term_from_int28((int32_t) actual_baudrate));
...
static term nif_uart_set_format(Context *ctx, int argc, term argv[])
{
    ...
    int data_bits = term_to_int(argv[1]);
    int stop_bits = term_to_int(argv[2]);
...
static term nif_uart_is_writable(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    ...
    bool writable = uart_is_writable(rsrc_obj->uart_inst);
    return writable ? TRUE_ATOM : FALSE_ATOM;
}

static term nif_uart_tx_wait_blocking(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    ...
    uart_tx_wait_blocking(rsrc_obj->uart_inst);

    return OK_ATOM;
}

static term nif_uart_set_fifo_enabled(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    ...
    bool enabled = (argv[1] == TRUE_ATOM);
    uart_set_fifo_enabled(rsrc_obj->uart_inst, enabled);

    return OK_ATOM;
}

static term nif_uart_set_break(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    ...
    bool en = (argv[1] == TRUE_ATOM);
    uart_set_break(rsrc_obj->uart_inst, en);

    return OK_ATOM;
}

static term nif_uart_putc_raw(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    ...
    int c_val = term_to_int(argv[1]);
    if (UNLIKELY(c_val < 0 || c_val > 255)) {
        RAISE_ERROR(BADARG_ATOM);
    }
    char c = (char) c_val;
    uart_putc_raw(rsrc_obj->uart_inst, c);

    return OK_ATOM;
}

static term nif_uart_putc(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    ...
    char c = (char) c_val;
    uart_putc(rsrc_obj->uart_inst, c);

    return OK_ATOM;
}

static term nif_uart_puts(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    ...
    for (size_t i = 0; i < len; i++) {
        uart_putc(rsrc_obj->uart_inst, data[i]);
    }

    return OK_ATOM;
}

static term nif_uart_getc(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    ...
    char c = uart_getc(rsrc_obj->uart_inst);

    if (UNLIKELY(memory_ensure_free(ctx, TUPLE_SIZE(2)) != MEMORY_GC_OK)) {
        RAISE_ERROR(OUT_OF_MEMORY_ATOM);
    }
    return create_pair(ctx, OK_ATOM, term_from_int11((int16_t) (uint8_t) c));
}
...
static term nif_uart_is_readable_within_us(Context *ctx, int argc, term argv[])
{
    ...
    int us_val = term_to_int(argv[1]);
    if (UNLIKELY(us_val < 0)) {
        RAISE_ERROR(BADARG_ATOM);
    }
    uint32_t us = (uint32_t) us_val;
...
static const struct Nif uart_is_writable_nif = {
    .base.type = NIFFunctionType,
    .nif_ptr = nif_uart_is_writable
};
static const struct Nif uart_tx_wait_blocking_nif = {
    .base.type = NIFFunctionType,
    .nif_ptr = nif_uart_tx_wait_blocking
};
static const struct Nif uart_set_fifo_enabled_nif = {
    .base.type = NIFFunctionType,
    .nif_ptr = nif_uart_set_fifo_enabled
};
static const struct Nif uart_set_break_nif = {
    .base.type = NIFFunctionType,
    .nif_ptr = nif_uart_set_break
};
static const struct Nif uart_putc_raw_nif = {
    .base.type = NIFFunctionType,
    .nif_ptr = nif_uart_putc_raw
};
static const struct Nif uart_putc_nif = {
    .base.type = NIFFunctionType,
    .nif_ptr = nif_uart_putc
};
static const struct Nif uart_puts_nif = {
    .base.type = NIFFunctionType,
    .nif_ptr = nif_uart_puts
};
static const struct Nif uart_getc_nif = {
    .base.type = NIFFunctionType,
    .nif_ptr = nif_uart_getc
};
...
static const struct Nif *uart_nif_get_nif(const char *nifname)
{
    ...
    if (strcmp("is_writable/1", rest) == 0) {
        TRACE("Resolved uart nif %s ...\n", nifname);
        return &uart_is_writable_nif;
    }
    if (strcmp("tx_wait_blocking/1", rest) == 0) {
        TRACE("Resolved uart nif %s ...\n", nifname);
        return &uart_tx_wait_blocking_nif;
    }
    if (strcmp("set_fifo_enabled/2", rest) == 0) {
        TRACE("Resolved uart nif %s ...\n", nifname);
        return &uart_set_fifo_enabled_nif;
    }
    if (strcmp("set_break/2", rest) == 0) {
        TRACE("Resolved uart nif %s ...\n", nifname);
        return &uart_set_break_nif;
    }
    if (strcmp("putc_raw/2", rest) == 0) {
        TRACE("Resolved uart nif %s ...\n", nifname);
        return &uart_putc_raw_nif;
    }
    if (strcmp("putc/2", rest) == 0) {
        TRACE("Resolved uart nif %s ...\n", nifname);
        return &uart_putc_nif;
    }
    if (strcmp("puts/2", rest) == 0) {
        TRACE("Resolved uart nif %s ...\n", nifname);
        return &uart_puts_nif;
    }
    if (strcmp("getc/1", rest) == 0) {
        TRACE("Resolved uart nif %s ...\n", nifname);
        return &uart_getc_nif;
    }
