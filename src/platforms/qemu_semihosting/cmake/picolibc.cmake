...
# Build picolibc from source with semihosting enabled for QEMU testing.
...
# Determine the compiler and architecture for the meson cross-file
if (CMAKE_SYSTEM_PROCESSOR STREQUAL "cortex-m3")
    set(_meson_cpu_family "arm")
    set(_meson_cpu "cortex-m3")
    set(_meson_c_args "'-mcpu=cortex-m3', '-mthumb'")
    set(_meson_compiler "${CMAKE_C_COMPILER}")
    # Find the other tools from the same prefix
    get_filename_component(_toolchain_dir "${CMAKE_C_COMPILER}" DIRECTORY)
    get_filename_component(_toolchain_prefix "${CMAKE_C_COMPILER}" NAME)
    string(REGEX REPLACE "gcc$" "" _toolchain_prefix "${_toolchain_prefix}")
    find_program(_meson_ar "${_toolchain_prefix}ar" HINTS "${_toolchain_dir}")
    find_program(_meson_as "${_toolchain_prefix}as" HINTS "${_toolchain_dir}")
    find_program(_meson_nm "${_toolchain_prefix}nm" HINTS "${_toolchain_dir}")
    find_program(_meson_strip "${_toolchain_prefix}strip" HINTS "${_toolchain_dir}")
elseif (CMAKE_SYSTEM_PROCESSOR STREQUAL "riscv32")
    set(_meson_cpu_family "riscv32")
    set(_meson_cpu "riscv32")
    set(_meson_c_args "'-march=rv32imac', '-mabi=ilp32'")
    set(_meson_compiler "${CMAKE_C_COMPILER}")
    get_filename_component(_toolchain_dir "${CMAKE_C_COMPILER}" DIRECTORY)
    get_filename_component(_toolchain_prefix "${CMAKE_C_COMPILER}" NAME)
    string(REGEX REPLACE "gcc$" "" _toolchain_prefix "${_toolchain_prefix}")
    ...
    message(FATAL_ERROR "Unsupported processor for QEMU semihosting picolibc: ${CMAKE_SYSTEM_PROCESSOR}")
endif()
...
ExternalProject_Add(picolibc
    ...
    BUILD_BYPRODUCTS
        ...
        "${PICOLIBC_INSTALL_DIR}/lib/libsemihost.a"
        "${PICOLIBC_INSTALL_DIR}/lib/crt0-semihost.o"
)
