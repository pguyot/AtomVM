...
static void websocket_dtor(ErlNifEnv *caller_env, void *obj)
{
    ...
    if (!websocket_rsrc->closed) {
        emscripten_websocket_close(websocket_rsrc->websocket, CLOSE_STATUS_CODE_GOING_AWAY, "");
    }
...
static void websocket_down(ErlNifEnv *caller_env, void *obj, ErlNifPid *pid, ErlNifMonitor *mon)
{
    ...
    printf("[%s:%d] %p ref_count = %d\n", __FUNCTION__, __LINE__, (void *) obj, (int) refc_binary_from_data(obj)->ref_count);
}
...
static term nif_websocket_new(Context *ctx, int argc, term argv[])
{
    ...
    printf("[%s:%d] ref_count = %d\n", __FUNCTION__, __LINE__, (int) refc_binary_from_data(rsrc_obj)->ref_count);
...
static term nif_websocket_send(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    ...
    if (term_is_binary(argv[1])) {
        ...
        int ok;
        char *text = interop_term_to_string(argv[2], &ok);
        if (!ok) {
            RAISE_ERROR(BADARG_ATOM);
        }
...
static const struct Nif websocket_send_nif = {
    .base.type = NIFFunctionType,
    .nif_ptr = nif_websocket_send
};
...
const struct Nif *websocket_get_nif(const char *nifname)
{
    ...
    if (strcmp("send/2", nifname) == 0) {
        return &websocket_send_nif;
    }
