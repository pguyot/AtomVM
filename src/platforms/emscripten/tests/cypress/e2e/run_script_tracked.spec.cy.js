...
describe("run_script_tracked", () => {
  ...
  it("should track, fetch and garbage collect JS values", () => {
    ...
    cy.window({ timeout: 10000 }).should((win) => {
      expect(win.Module.trackedObjectsMap.size).to.be.at.most(win.gcBaseline);
      const values = [...win.Module.trackedObjectsMap.values()];
      expect(values).to.include.members(["atom", "vm", "keep"]);
      expect(values).to.not.include.members(["g1", "g2", "g3"]);
    });
  });
});
