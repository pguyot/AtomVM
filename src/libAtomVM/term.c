...
int term_funprint(PrinterFun *fun, term t, const GlobalContext *global)
{
    if (term_is_atom(t)) {
        ...
        // Print no empty string ahead of the trailing bits field of a
        // sub-byte bitstring (<<1:1>>, not <<""1:1>>)
        int is_printable = (len != 0) || (trailing_bits == 0);
        for (int i = 0; i < len; i++) {
        ...
        if (trailing_bits != 0) {
            // Print the trailing partial byte as a final Value:Size field.
...
static inline TermCompareResult term_exact_equals(term t, term other, GlobalContext *global)
{
    ...
    // identical-or-immediate. Shared with the callers that run it inline
    // before deciding to call here at all (see term_exact_equals_shallow).
    bool shallow;
...
{
    // Backstop for any path that did not check TERM_MAX_BINARY_SIZE itself: an
    // oversized binary would make sub-binary offsets and term_bit_size wrap.
    // Heap binaries are always smaller than REFC_BINARY_MIN, so checking here
    // covers every binary size that can reach the limit.
...
term term_reuse_binary(term src, size_t size, Heap *heap, GlobalContext *glb)
{
    if (term_is_sub_binary(src)) {
        // A bitstring accumulator is a sub-binary wrapping its own storage at
        // offset 0: it is either created by bs_init_writable or returned by a
        // previous private_append, and the OTP compiler only emits
        // private_append for such uniquely referenced accumulators. Reuse the
        // underlying binary so that the trailing bits held by the wrapper are
        // kept, rather than copying whole bytes only.
...
_Static_assert(TERM_MAX_BINARY_SIZE <= SIZE_MAX >> 3,
    "a binary offset must survive being shifted left by 3");
