...
// A shrink is only worth a realloc once the slack exceeds this much; below it
// the allocation is released back rather than held forever by a binary that
// grew large and was then reused at a small size.
#define REUSE_BINARY_SHRINK_THRESHOLD (4 * REUSE_BINARY_MAX_SLACK)
...
term term_reuse_binary(term src, size_t size, Heap *heap, GlobalContext *glb)
{
    if (term_is_refc_binary(src) && !term_refc_binary_is_const(src)) {
        ...
        if (old_refc->ref_count == 1) {
            // The allocated capacity can exceed the logical size stored in the
            // boxed term: when the new size fits without leaving an unreasonable
            // amount of slack, just expose more (or less) of the allocation.
            if (size <= old_refc->capacity
                && old_refc->capacity - size < REUSE_BINARY_SHRINK_THRESHOLD) {
                if (size > old_size) {
