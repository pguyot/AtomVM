...
void context_dump(Context *ctx)
{
    ...
    fprintf(stderr, "has_min_heap_size: %d, min_heap_size: %zu\n", ctx->has_min_heap_size, ctx->min_heap_size);
    fprintf(stderr, "has_max_heap_size: %d, max_heap_size: %zu\n", ctx->has_max_heap_size, ctx->max_heap_size);
    fprintf(stderr, "trap_exit: %d\n", ctx->trap_exit);
...

    {
        ...
        int offset;
        long mod_offset;
        stacktrace_cp_to_mod_lbl_off(ctx->cp, ctx, &cp_mod, &label, &offset, &mod_offset);
