    ...
    enum BitstringFlags bs_flags, union maybe_unsigned_int64 *dst)
{
    ...
    if ((bs_flags & SignedInteger) && (out & ((uint64_t) 1) << (i - 1))) {
        // i == 64 leaves no sign bits to extend, and shifting by the type width is UB.
...
// Write the low n bits of value into dst starting at bit offset, MSB first.
// Only set bits are written: the destination is assumed to be zero-filled.
...
bool bitstring_insert_any_integer(uint8_t *dst, avm_int_t offset, avm_int64_t value, size_t n, enum BitstringFlags bs_flags)
{
    ...
    if (little_endian && (offset & 0x7) == 0 && (n & 0x7) == 0) {
        ...
        for (size_t i = 0; i < whole_bytes; ++i) {
            insert_bits_msb_first(dst, offset + 8 * i, uvalue & 0xFF, 8);
            uvalue >>= 8;
        }
        if (rem != 0) {
            insert_bits_msb_first(dst, offset + 8 * whole_bytes, uvalue, rem);
        }
    } else {
        // Big-endian: write bits MSB first
...
static bool is_invalid_codepoint(int64_t v)
{
...
bool bitstring_utf8_encode(int64_t c, uint8_t *buf, size_t *out_size)
{
    size_t sz = 0;
...
bool bitstring_utf16_encode(int64_t c, uint8_t *buf, enum BitstringFlags bs_flags, size_t *out_size)
{
    size_t sz = 0;
    if (is_invalid_codepoint(c)) {
        return false;
...
bool bitstring_utf32_encode(int64_t c, uint8_t *buf, enum BitstringFlags bs_flags)
{
    UNUSED(bs_flags);
    if (is_invalid_codepoint(c)) {
        return false;
...
void bitstring_copy_bits_incomplete_bytes(uint8_t *dst, size_t bits_offset, const uint8_t *src, size_t bits_count)
{
    ...
    // Eventually copy bit by bit. Only the bytes that hold a copied bit may be
    // read or written: the destination may be allocated to exactly the size of
    // the bits it holds (a refc binary is), and the last requested bit may fall
    // exactly on a byte boundary of either buffer.
    dst += byte_offset;
