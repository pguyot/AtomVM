...
{
    BEGIN_WITH_STACK_HEAP(TUPLE_SIZE(2) + EXTERNAL_PID_SIZE, heap)
...
static term nif_erlang_setnode_3(Context *ctx, int argc, term argv[])
{
    ...
    int32_t process_pid = term_to_local_process_id(argv[1]);
    if (UNLIKELY(enif_monitor_process(env, conn_obj, &process_pid, &conn_obj->connection_process_monitor) != 0)) {
        synclist_unlock(&ctx->global->dist_connections);
        RAISE_ERROR(BADARG_ATOM);
    }
...
static term nif_erlang_dist_ctrl_get_data_notification(Context *ctx, int argc, term argv[])
{
    ...
    synclist_wrlock(&conn_obj->pending_packets);
    ...
    switch (term_to_int(operation)) {
        ...
        case OPERATION_MONITOR_P: {
            ...
            if (target_process_id) {
                ...
                if (UNLIKELY(enif_monitor_process(erl_nif_env_from_context(ctx), conn_obj, &target_process_id, &monitor->process_monitor) != 0)) {
