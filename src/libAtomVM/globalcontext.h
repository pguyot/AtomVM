...
// Shard locks are word-sized atomic read-write locks where available (pid
// lookups are a few loads; the only long write hold is context teardown,
// and spinners yield). Platforms without lock-free atomics (e.g. armv6m,
// where every CAS is a critical section) keep the platform RWLock.
