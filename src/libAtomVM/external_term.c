...
static int serialize_simple_term(uint8_t *buf, term t, GlobalContext *glb)
{
    if (term_is_uint8(t)) {
        ...
        if (term_is_byte_aligned_binary(t)) {
            ...
            // Non-byte-aligned bitstring: BIT_BINARY_EXT with the valid-bit count
            // of the trailing byte (1..7 here).
    ...
    switch (external_term_buf[0]) {
        ...
        case BIT_BINARY_EXT: {
            ...
            *eterm_size = 6 + binary_size;
            ...
            // last_byte_bits in 1..8; 8 (or empty) means byte-aligned.
