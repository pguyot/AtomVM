    ...
    uintptr_t load_address, size_t *new_elf_size)
{
    ...
    size_t elf_size = sizeof(Elf_Ehdr) + sizeof(Elf_Phdr) + (section_count * sizeof(Elf_Shdr)) + code_size + symtab_size + strtab_size + shstrtab_size + debug_info_size + debug_line_size + debug_abbrev_size + debug_str_size + debug_aranges_size;
    ...
    for (size_t i = 0; i < num_syms && i < 10; i++) {
        if (ELF_ST_TYPE(syms[i].st_info) == STT_FUNC) {
            UNUSED(new_strtab);
            TRACE("  Symbol[%zu]: %s @ 0x%lx (size=%zu)\n", i, (const char *) (new_strtab + syms[i].st_name),
        ...
        if (debug_aranges_size >= 12) {
            ...
            if (debug_aranges_size >= descriptor_offset + addr_size * 2) {
                if (addr_size == 8) {
                    ...
                    TRACE("Patched .debug_aranges: 0x%llx -> ", (unsigned long long) *range_start);
                    *range_start += load_address;
                    TRACE("0x%llx\n", (unsigned long long) *range_start);
                } else if (addr_size == 4) {
                    uint32_t *range_start = (uint32_t *) (new_debug_aranges + descriptor_offset);
                    TRACE("Patched .debug_aranges: 0x%x -> ", *range_start);
