...
{
    UNUSED(global);
    return hash_uint64((uint64_t) term_maybe_unbox_int64(t), h, LARGE_PRIME_INTEGER);
}
...
{
    UNUSED(global);
    return hash_uint32((uint32_t) term_to_local_process_id(t), h, LARGE_PRIME_PID);
}

static uint32_t hash_local_port(term t, uint32_t h, GlobalContext *global)
{
    UNUSED(global);
    return hash_uint32((uint32_t) term_to_local_process_id(t), h, LARGE_PRIME_PORT);
}

static uint32_t hash_external_pid(term t, uint32_t h, GlobalContext *global)
{
    UNUSED(global);
    return hash_uint32((uint32_t) term_get_external_pid_process_id(t), h, LARGE_PRIME_PID);
}

static uint32_t hash_external_port(term t, uint32_t h, GlobalContext *global)
{
    UNUSED(global);
    return hash_uint32((uint32_t) term_get_external_port_number(t), h, LARGE_PRIME_PORT);
}

static uint32_t hash_local_reference(term t, uint32_t h, GlobalContext *global)
{
    UNUSED(global);
    return hash_uint64(term_to_ref_ticks(t), h, LARGE_PRIME_REF);
}
...
static uint32_t hash_function(term t, uint32_t h, GlobalContext *global)
{
    const term *boxed_value = term_to_const_term_ptr(t);

    if (term_is_external_fun(t)) {
        h = h * LARGE_PRIME_FUNCTION + 1;
        h = h * LARGE_PRIME_FUNCTION + hash_term_incr(boxed_value[1], h, global);
        h = h * LARGE_PRIME_FUNCTION + hash_term_incr(boxed_value[2], h, global);
        h = h * LARGE_PRIME_FUNCTION + hash_term_incr(boxed_value[3], h, global);
...
static uint32_t hash_term_incr(term t, uint32_t h, GlobalContext *global)
{
    if (term_is_atom(t)) {
        ...
        return hash_function(t, h, global);
