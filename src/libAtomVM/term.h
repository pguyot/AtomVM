...
/**
 * @brief Largest representable binary size, in bytes.
 *
 * @details A sub-binary packs its byte offset and its trailing bit count into a
 * single word as `(offset << 3) | trailing_bits`, and `term_bit_size` computes
 * `byte_size * 8 + trailing_bits`. Both require byte sizes and offsets to fit in
 * a word once multiplied by 8. Binaries are therefore capped at `SIZE_MAX >> 3`
 * bytes: unreachable on 64-bit, slightly under 512 MiB on 32-bit. Larger sizes
 * are rejected with `system_limit`, as OTP does for unrepresentable binaries.
 */
#define TERM_MAX_BINARY_SIZE (SIZE_MAX >> 3)
 ...
 *
 * @details Try to reuse a binary and return a term pointing to it. If src is a
 * bitstring, its storage is reused as a whole, trailing bits included: src must
 * then be a sub-binary at offset 0 over its own storage, which is what a
 * private_append accumulator always is.
 * @param src binary or bitstring to reuse.
 * @param size size of binary data buffer, in bytes.
 * @param heap the heap to allocate memory in
 * @param glb the global context as refc binaries are global
 * @return a term pointing to the boxed binary pointer, or `term_invalid_term()`
