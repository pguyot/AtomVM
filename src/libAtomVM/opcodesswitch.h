...
#define DECODE_DEST_REGISTER_UNSAFE(dreg, decode_pc) \
...
#define DECODE_DEST_REGISTER_UNSAFE(dreg_unsafe, decode_pc)                                                     \
{                                                                                                               \
    uint8_t first_byte = *(decode_pc)++;                                                                        \
    uint8_t reg_type = first_byte & 0xF;                                                                        \
    uint8_t reg_index = (first_byte >> 4);                                                                      \
    switch (reg_type) {                                                                                         \
        ...
        case COMPACT_LARGE_YREG:                                                                                \
            if (LIKELY((first_byte & COMPACT_LARGE_IMM_MASK) == COMPACT_11BITS_VALUE)) {                        \
                (dreg_unsafe).base = ctx->e;                                                                    \
                (dreg_unsafe).index = (((first_byte & 0xE0) << 3) | *(decode_pc)++);                            \
            } else {                                                                                            \
                VM_ABORT();                                                                                     \
            }                                                                                                   \
            break;                                                                                              \
           ...
           case OP_IS_FUNCTION: {
            ...
            case OP_BS_START_MATCH3: {
                ...
                DECODE_DEST_REGISTER_UNSAFE(dreg, pc);
                    ...
                    if (!(term_is_binary(src) || term_is_match_state(src))) {
                        WRITE_REGISTER_UNSAFE(dreg, src);
                        pc = mod->labels[fail];
                    } else {
                        term match_state = term_alloc_bin_match_state(src, 0, &ctx->heap);

                        WRITE_REGISTER_UNSAFE(dreg, match_state);
                    }
                #endif
                break;
            }
