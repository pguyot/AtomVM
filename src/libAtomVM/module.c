...
#ifdef JIT_JUMPTABLE_IS_DATA
        // WASM: entry_point is not used; the dispatch loop uses
        // JIT_CONTINUATION_FOR_LABEL(target, label) which stores (label + 1)
        // and resolves per-thread function pointers lazily.
        mfunc->entry_point = NULL;
#else
