...
b_w_test_() ->
    [
        ...
        % b.w .-4096
        ?_assertAsmEqual(
            <<16#F7FF:16/little, 16#B000:16/little>>,
            "b.w .-4092",
            jit_armv7m_asm:b_w(-4096)
        )
    ].

%% B<cc>.W (32-bit conditional branch) tests
bcc_w_test_() ->
    [
        % beq.w .+0
        ?_assertAsmEqual(
            <<16#F000:16/little, 16#8000:16/little>>,
            "beq.w .+4",
            jit_armv7m_asm:bcc_w(eq, 0)
        ),
        % bne.w .+4
        ?_assertAsmEqual(
            <<16#F040:16/little, 16#8002:16/little>>,
            "bne.w .+8",
            jit_armv7m_asm:bcc_w(ne, 4)
        ),
        % beq.w .-4
        ?_assertAsmEqual(
            <<16#F3FF:16/little, 16#AFFE:16/little>>,
            "beq.w .",
            jit_armv7m_asm:bcc_w(eq, -4)
        ),
        % bge.w .+256
        ?_assertAsmEqual(
            <<16#F2C0:16/little, 16#8080:16/little>>,
            "bge.w .+260",
            jit_armv7m_asm:bcc_w(ge, 256)
        ),
        % blt.w .-256
        ?_assertAsmEqual(
            <<16#F37F:16/little, 16#AF80:16/little>>,
            "blt.w .-252",
            jit_armv7m_asm:bcc_w(lt, -256)
        ),
        % bcs.w .+1024
        ?_assertAsmEqual(
            <<16#F080:16/little, 16#8200:16/little>>,
            "bcs.w .+1028",
            jit_armv7m_asm:bcc_w(cs, 1024)
        )
    ].
...
movw_test_() ->
    [
        ...
        % movw r12, #0xABCD
        ?_assertAsmEqual(
            <<16#F64B:16/little, 16#3CCD:16/little>>,
            "movw r12, #0xABCD",
            jit_armv7m_asm:movw(r12, 16#ABCD)
        ),
        % movw r1, #0x800 (test i bit)
        ?_assertAsmEqual(
            <<16#F240:16/little, 16#0100:16/little>>,
            "movw r1, #2048",
...
movt_test_() ->
    [
        ...
        % movt r12, #0xABCD
        ?_assertAsmEqual(
            <<16#F6CB:16/little, 16#3CCD:16/little>>,
            "movt r12, #0xABCD",
