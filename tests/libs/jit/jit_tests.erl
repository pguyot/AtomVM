...
term_to_int_verify_is_match_state_typed_optimization_x86_64_test() ->
    ...
    % shift (sar), so a negative size stays negative. ctx is pinned in r14,
    % and the tag-test's conditional jump to the fail label is fused into a
    % single jne rel32:
    % 15c:	4d 8b 5e 60          	mov    0x60(%r14),%r11
    % 160:	4d 89 da             	mov    %r11,%r10
    % 163:	41 80 e2 0f          	and    $0xf,%r10b
    % 167:	41 80 fa 0f          	cmp    $0xf,%r10b
    % 16b:	0f 85 xx xx xx xx    	jne    <fail>
    % 171:	49 c1 fb 04          	sar    $0x4,%r11
    ?assertMatch(
        {_, _},
        binary:match(
            CompiledCode,
            <<16#4d, 16#8b, 16#5e, 16#60, 16#4d, 16#89, 16#da, 16#41, 16#80, 16#e2, 16#0f, 16#41,
                16#80, 16#fa, 16#0f, 16#0f, 16#85>>
        )
    ),
...
if_block_unsigned_less_than_test_() ->
    ...
    [
        {atom_to_list(Backend), fun() ->
            ...
            State3 = Backend:if_block(State2, {RegA, '<u', RegB}, fun(BSt0) ->
