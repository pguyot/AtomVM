...
elf_generation_test() ->
    ...
    case jit_dwarf:elf(State3, <<>>) of
        false ->
            ok;
        {ok, _TextSectionOffset, ElfBinary} ->
            % Verify ELF magic
...
section_header_test() ->
    ...
    case jit_dwarf:elf(State1, <<>>) of
        ...
        {ok, _TextSectionOffset, ElfBinary} ->
            ...
            % All zeros
            ?assertEqual(binary:copy(<<0>>, 40), NullHeader)
    end.
...
elf_with_text_test() ->
    ...
    case jit_dwarf:elf(State, NativeCode) of
        false ->
            ok;
        {ok, _TextSectionOffset, CombinedELF} ->
            % Verify ELF magic
            <<127, $E, $L, $F, _Rest/binary>> = CombinedELF,

            % Parse 64-bit ELF header to check section count
            % (null + .text + 5 debug sections + .symtab + .strtab + .shstrtab = 10)
            <<_ElfMagic:16/binary, _Type:16/little, _Machine:16/little, _Version:32/little,
                _Entry:64/little, _PHOff:64/little, _SHOff:64/little, _Flags:32/little,
                _EHSize:16/little, _PHEntSize:16/little, _PHNum:16/little, _SHEntSize:16/little,
                SHNum:16/little, _SHStrNdx:16/little, _/binary>> = CombinedELF,

            % Should have 10 sections total
            ?assertEqual(10, SHNum),
            ...
            {ok, _, DebugOnlyELF} = jit_dwarf:elf(State, <<>>),
            ?assert(byte_size(CombinedELF) > byte_size(DebugOnlyELF))
    end.

text_section_properties_test() ->
    % Test that the .text section has proper properties (use armv6m for 32-bit ELF)
    State = jit_dwarf:new(jit_armv6m, test_module, jit_stream_binary, 1024),

    % Some dummy native code
    NativeCode = <<16#00, 16#bf, 16#00, 16#bf>>,

    case jit_dwarf:elf(State, NativeCode) of
        false ->
            ok;
        {ok, _TextSectionOffset, CombinedELF} ->
            % Parse 32-bit ELF to find .text section
            <<_ElfMagic:16/binary, _Type:16/little, _Machine:16/little, _Version:32/little,
                _Entry:32/little, _PHOff:32/little, SHOff:32/little, _Flags:32/little,
                _EHSize:16/little, _PHEntSize:16/little, _PHNum:16/little, _SHEntSize:16/little,
