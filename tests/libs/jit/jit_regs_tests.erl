...
new_test() ->
    ...
    ?assertEqual(unknown, jit_regs:get_type(Regs, {x_reg, 0})),
...
type_tracking_test() ->
    Regs0 = jit_regs:new(),
    Regs1 = jit_regs:set_type(Regs0, {x_reg, 0}, integer),
    ?assertEqual(integer, jit_regs:get_type(Regs1, {x_reg, 0})),
    ?assertEqual(unknown, jit_regs:get_type(Regs1, {x_reg, 1})),
    Regs2 = jit_regs:set_type(Regs1, {y_reg, 0}, atom),
    ?assertEqual(atom, jit_regs:get_type(Regs2, {y_reg, 0})),
    Regs3 = jit_regs:invalidate_type(Regs2, {x_reg, 0}),
    ?assertEqual(unknown, jit_regs:get_type(Regs3, {x_reg, 0})),
    ?assertEqual(atom, jit_regs:get_type(Regs3, {y_reg, 0})).

invalidate_all_types_test() ->
    Regs0 = jit_regs:new(),
    Regs1 = jit_regs:set_type(Regs0, {x_reg, 0}, integer),
    Regs2 = jit_regs:set_type(Regs1, {y_reg, 0}, atom),
    Regs3 = jit_regs:invalidate_all_types(Regs2),
    ?assertEqual(unknown, jit_regs:get_type(Regs3, {x_reg, 0})),
    ?assertEqual(unknown, jit_regs:get_type(Regs3, {y_reg, 0})).

merge_test() ->
    Regs0 = jit_regs:new(),
    %% Path A: rax = x_reg 0, r11 = x_reg 1, type x0 = integer
    RegsA = jit_regs:set_type(
        jit_regs:set_contents(
    ...
    %% Path B: rax = x_reg 0, r11 = x_reg 2, type x0 = integer, type x1 = atom
    RegsB = jit_regs:set_type(
        jit_regs:set_type(
            jit_regs:set_contents(
                jit_regs:set_contents(Regs0, rax, {x_reg, 0}),
                r11,
    ...
    %% x0 = integer in both: kept
    ?assertEqual(integer, jit_regs:get_type(Merged, {x_reg, 0})),
    %% x1 = atom only in B: invalidated
    ?assertEqual(unknown, jit_regs:get_type(Merged, {x_reg, 1})).
