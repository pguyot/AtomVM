...
test_controller_handshake_send_framing() ->
    %% Verify that send/2 adds a 2-byte big-endian length prefix
    {A, B} = mock_uart_hal:create_pair(),
...
test_controller_handshake_recv() ->
    %% Verify that recv/3 correctly parses a 2-byte length-prefixed packet
    {A, B} = mock_uart_hal:create_pair(),
    {ok, Ctrl} = serial_dist_controller:start(A, mock_uart_hal),
    %% Write a properly framed handshake packet to the peer side
    ok = mock_uart_hal:write(B, <<5:16, "hello">>),
...
test_controller_handshake_recv_fragmented() ->
    ...
    %% Send length prefix alone
    receive
    after 50 -> ok
    end,
    ok = mock_uart_hal:write(B, <<5:16>>),
    %% Send partial payload
    receive
    after 50 -> ok
    end,
    ok = mock_uart_hal:write(B, <<"hel">>),
    %% Send remaining payload
    receive
    after 50 -> ok
    end,
    ok = mock_uart_hal:write(B, <<"lo">>),
    receive
...
test_controller_tick() ->
    %% Verify tick sends exactly 4 zero bytes (32-bit zero)
    {A, B} = mock_uart_hal:create_pair(),
    {ok, Ctrl} = serial_dist_controller:start(A, mock_uart_hal),
...
test_controller_initial_data() ->
    ...
    %% This simulates the accept path where accept_loop reads the first
    %% chunk from UART and passes it to the controller.
    {A, B} = mock_uart_hal:create_pair(),
    InitialData = <<5:16, "hello">>,
    ...
    ok = mock_uart_hal:write(B, <<3:16, "abc">>),
