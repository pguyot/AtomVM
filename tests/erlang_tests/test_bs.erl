...
% A dynamic segment size comes from a register and can be negative. It must be
% rejected before it is scaled by the segment unit: the scaled size is compared
% against the remaining capacity and added to the match offset, so scaling
% first lets a negative size wrap to a small one and match, or move the match
% offset before the start of the binary.
