...
start() ->
    ...
    %% binary_to_list/1 requires a byte-aligned binary (unlike bitstring_to_list/1).
    ok = raises_badarg(fun() -> erlang:binary_to_list(?ID(<<1:1>>)) end),
    ok = raises_badarg(fun() -> erlang:binary_to_list(?ID(<<255, 5:7>>)) end),
